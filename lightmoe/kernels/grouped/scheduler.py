...
class MoEPersistentTileScheduler:
    ...
    def __init__(
        ...
        dynamic_state: _DynamicSchedState | None = None,
        sched_storage=None,
    ...
    def __new_from_mlir_values__(self, values: list[ir.Value]) -> "MoEPersistentTileScheduler":
        ...
        if self.params.use_dynamic_sched:
            ...
            new_dynamic_state = new_from_mlir_values(
                self._dynamic_state, values[idx : idx + ds_len]
            )
            idx += ds_len
    ...
    def create(
        ...
        counter_ptr: cute.Pointer | None = None,
        sched_storage=None,
        *,
        loc=None,
        ip=None,
    ...
    def initial_work_tile_info(self, *, loc=None, ip=None) -> MoEWorkTileInfo:
        ...
        if const_expr(self.use_dynamic_sched):
            if const_expr(not hasattr(self, "cluster_pipeline")):
                raise RuntimeError(
                    "Make sure sched.internal_init() is called at the barrier init place before used."
                )
            self._current_work_linear_idx = self._fetch_next_cluster_idx(loc=loc, ip=ip)
            work_tile_info = self._get_work_tile_for_linear_idx(
    ...
    def _fetch_next_cluster_idx(self, *, loc=None, ip=None) -> Int32:
        ...
        if ds.is_leader_cta:
            self.cluster_pipeline.producer_acquire(ds.producer_state)
            full_barrier_ptr = self.cluster_pipeline.sync_object_full.get_barrier(
    ...
    def _cluster_producer_commit(self, *, loc=None, ip=None) -> None:
        ...
        if ds.is_leader_cta and lane_idx < Int32(cluster_size):
            mbarrier_arrive_expect_tx_on_peer(
                self.cluster_pipeline.sync_object_full.get_barrier(
                    ds.producer_state.index, loc=loc, ip=ip
                ),
                Int32(4),
                lane_idx,
    ...
    def _get_work_tile_for_linear_idx(
        ...
        if is_valid:
            ...
            cta_tile_m_idx = (
                cluster_tile_m_idx * self.cluster_shape_mn[0] + self.cta_id_in_cluster[0]
            )
            cta_tile_n_idx = (
                cluster_tile_n_idx * self.cluster_shape_mn[1] + self.cta_id_in_cluster[1]
            )
    ...
    def _advance_expert_to_contain(
        ...
        *,
        ...
        # Advance until the requested linear tile falls inside the current expert.
        while (
            cluster_linear_idx >= self.expert_tile_end and self.current_expert_idx < self.expert_cnt
        ):
            self.current_expert_idx += 1
            self.expert_tile_start = self.expert_tile_end
            if self.current_expert_idx < self.expert_cnt:
                tile_count = self._compute_tiles_for_expert(
                    self.current_expert_idx,
                    loc=loc,
                    ip=ip,
                )
                self.expert_tile_end += tile_count
