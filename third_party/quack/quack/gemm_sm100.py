...
class GemmSm100(GemmTmaBase):
    ...
    def __init__(
        ...
        # Diagnostic ONLY: compile the sfa_linear kernel without issuing the SF
        # cp.async gather (SF smem is stale garbage -> wrong results). Used to
        # attribute producer-side time between the A gather and the SF gather.
        debug_skip_sfa_gather: bool = False,
        # sfa_linear SF panel (Q28): >0 hoists the gathered SFA out of the AB
        # pipeline into ONE per-work-tile buffer holding this many k-tile slots
        # (must equal ceil(K / mma_tiler_k) — it sizes smem and is part of the
        # jit key, so a different K must compile a different kernel). 0 = the
        # per-stage loader. sfa_hoist_pair picks the row-major (one warp
        # instruction per gathered row) fill; see _make_hoisted_SFA_copy.
        sfa_hoist_slots: int = 0,
        sfa_hoist_pair: bool = False,
        # sfa_linear SF fill shape (Q29), inside the unchanged per-stage AB
        # ring: 0 = the 4-B scattered cp.async loader; 1 = LDG.128 + register
        # transpose + STS.128 issued after the stage acquire; 2 = the same fill
        # with the global read moved one k-tile ahead of the acquire it feeds.
        # sfa_xpose_scatter is the falsification control (same LDG/barrier
        # rewiring, stores deliberately de-vectorized to 4-B STS).
        sfa_xpose: int = 0,
        sfa_xpose_scatter: bool = False,
        # sfa_linear SF ring (Q30): >0 takes sSFA out of the AB stage struct and
        # gives it its OWN circular buffer of this many slots behind its own
        # multi-stage barrier, so the SF fill's COMPLETION leaves the AB stage's
        # ready path (the AB full flip becomes TMA-only) and the SF warp's
        # run-ahead is no longer capped by ab_stage. The fill itself is the
        # unchanged cp.async loader (Q29: the LDGSTS bypass is load-bearing).
        # Unlike sfa_hoist this is NOT the k-tile count: the ring wraps, so any
        # depth >= 2 is legal for any K. 0 = SFA rides the AB stages.
        sfa_ring: int = 0,
    ):
        ...
        if gather_A:
            # cp.async gather cannot multicast; tma.gather4 can (.multicast::cluster),
            # so the TMA-gather path lifts the cluster-N restriction and shares the
            # gathered A rows across the N peers.
            assert cluster_shape_mnk[1] == 1 or use_tma_gather, (
                "gather_A requires cluster N == 1 unless use_tma_gather (multicast gather4)"
            )
        if use_tma_gather:
            assert gather_A, "TMA gather requires gather_A=True"
        # Blockscaled x gather_A: SFA arrives in the LINEAR per-row layout
        # (T, K/sf_vec) — viewed as (T, K/(4*sf_vec)) Int32 words at the FFI
        # boundary — and the cp.async producer warps gather it row-wise into
        # the blocked smem atom (see _make_gather_SFA_copy). The blocked gmem
        # layout cannot be row-gathered: row m of an atom lives at
        # atom[m%32][m//32][k%4], so an arbitrary-row gather would scatter
        # inside the 512 B atom. TMA-gather has no such linear-SF path.
        self.sfa_linear = self.blockscaled and gather_A
        self.debug_skip_sfa_gather = debug_skip_sfa_gather
        # Defaults only: the mod surface sets these through post_init_attrs
        # (after __init__), so validation lives in _setup_attributes.
        self.sfa_hoist_slots = int(sfa_hoist_slots)
        self.sfa_hoist_pair = bool(sfa_hoist_pair)
        self.sfa_xpose = int(sfa_xpose)
        self.sfa_xpose_scatter = bool(sfa_xpose_scatter)
        self.sfa_ring = int(sfa_ring)
        if self.sfa_linear and use_tma_gather:
            # The cp.async gather path forwards the non-leader CTA's async-copy
            # completion to the leader's barrier (need_nonleader_cta); the TMA
            # gather path has no such seam yet, so the SF producer warp's
            # cp.asyncs on the non-leader CTA would race the leader's 2-CTA MMA
            # (measured: ~2% mismatched outputs on a 256x256 (2,1) run).
            assert not self.use_2cta_instrs, (
                "blockscaled TMA-gather does not support 2-CTA MMA yet"
            )
    ...
    def _setup_attributes(self, epilogue_args: EpilogueArguments, varlen_args: VarlenArguments):
        ...
        if self.gather_A:
            assert self.num_mcast_ctas_a == 1 or self.use_tma_gather
        ...
        # SF panel (Q28): sSFA leaves the AB pipeline and becomes ONE per-work-tile
        # buffer with a slot per k-tile, filled once at tile start. It is then a
        # fixed smem cost instead of a per-AB-stage one, so the stage budget below
        # must account for it separately.
        hoist_slots = int(getattr(self, "sfa_hoist_slots", 0) or 0)
        self.sfa_hoist = hoist_slots > 0
        if const_expr(self.sfa_hoist):
            assert self.sfa_linear, "sfa_hoist_slots requires the linear-SF gather path"
            # The panel's empty barrier is released by the mma warp's tcgen05
            # commit; the 2-CTA seam (non-leader forwarding) is not wired for it.
            assert not self.use_2cta_instrs, "sfa_hoist does not support 2-CTA MMA yet"
        # SF ring (Q30): same "sSFA leaves the AB stage struct" accounting as the
        # panel, but the buffer is circular and the barrier has one stage per
        # slot, so the fill runs ahead by up to ring_slots k-tiles instead of
        # being a single per-work-tile buffer (Q28's failure mode).
        ring_slots = int(getattr(self, "sfa_ring", 0) or 0)
        self.sfa_ring_slots = ring_slots
        self.sfa_ring_on = ring_slots > 0
        if const_expr(self.sfa_ring_on):
            assert self.sfa_linear, "sfa_ring requires the linear-SF gather path"
            assert not self.sfa_hoist, "sfa_ring and sfa_hoist are alternative SF buffers"
            assert not int(getattr(self, "sfa_xpose", 0) or 0), (
                "sfa_ring and sfa_xpose are alternative SF fills"
            )
            assert ring_slots >= 2, (
                f"sfa_ring needs >= 2 slots (1 is Q28's single-buffered panel), got {ring_slots}"
            )
            assert self.cta_tile_shape_mnk[0] == 128, "sfa_ring requires a 128-row CTA tile"
            # The ring's empty barrier is released by the mma warp's tcgen05
            # commit; the 2-CTA non-leader forwarding seam is not wired for it.
            assert not self.use_2cta_instrs, "sfa_ring does not support 2-CTA MMA yet"
            # Producer and consumer walk the ring in lockstep by iterating the
            # same (tile, k-tile) sequence; split-K gives the mma warp a
            # different k-tile range than the SF warp's full one.
            assert self.split_k == 1, "sfa_ring does not support split-K"
        (
            self.num_acc_stage,
            self.ab_stage,
            self.epi_stage,
            ...
            # set via post_init (mod path: post_init_attrs; plain path:
            # gemm(stage_overrides=...)) - a tuning knob, part of the jit key
            overrides=getattr(self, "stage_overrides", None),
            sfa_panel_slots=hoist_slots or ring_slots,
        )
        # SFA slots: one per AB stage (default), one per k-tile (hoisted panel),
        # or ring_slots (the Q30 ring, decoupled from the AB stage count).
        self.sfa_slots = self.ab_stage
        if const_expr(self.sfa_hoist):
            self.sfa_slots = hoist_slots
        elif const_expr(self.sfa_ring_on):
            self.sfa_slots = ring_slots
        if const_expr(self.sfa_hoist):
            # Words of linear SF per gathered row over the WHOLE K the panel
            # covers: rk_words per k-tile x one slot per k-tile = K / (4*sf_vec).
            rk_words = self.mma_tiler[2] // (self.sf_vec_size * 4)
            self.sfa_panel_row_words = self.sfa_slots * rk_words
            if const_expr(self.sfa_hoist_pair):
                # The row-major fill hands one row's contiguous words to one warp
                # instruction, so a row must be an exact number of warp-wide runs.
                assert self.sfa_panel_row_words % cute.arch.WARP_SIZE == 0, (
                    f"sfa_hoist_pair needs K % {64 * cute.arch.WARP_SIZE} == 0 "
                    f"(row words {self.sfa_panel_row_words} not a multiple of 32)"
                )
        # Q29 transposed SF fill: same stage, same words, same bytes — only the
        # instruction shape and (mode 2) the issue point change, so it costs no
        # smem and leaves the stage budget above untouched.
        self.sfa_xpose = int(getattr(self, "sfa_xpose", 0) or 0)
        self.sfa_xpose_scatter = bool(getattr(self, "sfa_xpose_scatter", False))
        if const_expr(self.sfa_xpose):
            assert self.sfa_xpose in (1, 2), f"sfa_xpose must be 0/1/2, got {self.sfa_xpose}"
            assert self.sfa_linear, "sfa_xpose requires the linear-SF gather path"
            assert not self.sfa_hoist, "sfa_xpose and sfa_hoist are alternative SF fills"
            assert self.cta_tile_shape_mnk[0] == 128, "sfa_xpose requires a 128-row CTA tile"
            # The cp.async-A gather path routes the non-leader CTA's producer
            # arrives through a forwarding seam; a plain arrive should compose
            # with it the same way the cp.async one does, but that is untested
            # and this flag is an experiment. (TMA-gather x 2-CTA is already
            # blocked for sfa_linear above.)
            assert not self.use_2cta_instrs, "sfa_xpose does not support 2-CTA MMA yet"
            # One 16-B LDG must cover a row's whole k-tile SF run, and the four
            # rows a lane owns must land at four consecutive destination words:
            # both hold exactly at rk_words == 4 (nvfp4 sf_vec 16, tile_k 256).
            xpose_rk_words = self.mma_tiler[2] // (self.sf_vec_size * 4)
            assert xpose_rk_words == 4, (
                f"sfa_xpose needs 4 SF words per row per k-tile, got {xpose_rk_words} "
                f"(mma_tiler_k={self.mma_tiler[2]}, sf_vec={self.sf_vec_size})"
            )
        if os.environ.get("QUACK_LOG_STAGES"):
            print(
                f"[stages] tile={self.cta_tile_shape_mnk} acc={self.num_acc_stage} "
                f"ab={self.ab_stage} epi={self.epi_stage} epi_c={self.epi_c_stage}"
                + (f" sfa_panel={self.sfa_slots}" if self.sfa_hoist else "")
                + (f" sfa_ring={self.sfa_slots}" if self.sfa_ring_on else "")
                + (
                    f" sfa_xpose={self.sfa_xpose}"
                    + ("/scatter" if self.sfa_xpose_scatter else "")
                    if self.sfa_xpose
                    else ""
                ),
                flush=True,
    ...
    def __call__(
        ...
        if const_expr(self.blockscaled):
            ...
            # Linear SFA (gather_A) stays rank-2 (T, K/64) Int32.
            if const_expr(not self.sfa_linear and cute.rank(mSFA) == 5):
        ...
        # Linear SFA crosses the FFI boundary as Int32 words (torch-side view of
        # 4 e4m3/e8m0 bytes); the scale dtype then comes from SFB, whose blocked
        # tensor keeps the true element type.
        self.sf_dtype: Optional[Type[cutlass.Numeric]] = (
            (mSFB.element_type if const_expr(self.sfa_linear) else mSFA.element_type)
            if mSFA is not None
            else None
        )
        ...
        if const_expr(self.sfa_linear):
            # The SF gather runs on the (otherwise idle) epi-load warp; a C
            # operand / tile-load epilogue would need that warp for C.
            assert self.epi_c_stage == 0, (
                "blockscaled gather_A does not support a C operand / tile-load epilogue "
                "(the epi-load warp is repurposed as the SF producer)"
            )

        if const_expr(self.blockscaled and not self.sfa_linear):
        ...
        if const_expr(self.blockscaled):
            ...
            if const_expr(not self.sfa_linear):
                ...
                # Linear SFA rides the cp.async gather producer; the raw
                # (T, K/64) Int32 tensor goes to the kernel as-is.
                tma_atom_sfa, tma_tensor_sfa = None, mSFA
                # Fence: the gather's hand-built Int32 word view of the blocked
                # smem stage (see _make_gather_SFA_copy) must cover exactly the
                # bytes one stage owns: 512 B per K-atom, rk_words atoms.
                rk_words = self.mma_tiler[2] // (self.sf_vec_size * 4)
                assert (
                    cute.cosize(cute.filter_zeros(self.sfa_smem_layout_staged))
                    == 512 * rk_words * self.sfa_slots
                ), "blocked SFA smem layout does not match the linear-SF word view"
        ...
        if const_expr(self.blockscaled):
            ...
            self.num_tma_load_bytes += sfb_copy_size
            if const_expr(not self.sfa_linear):
                # Linear SFA is cp.async-gathered: its completion is tracked by
                # the producer lanes' cp.async arrives, not the TMA tx count.
                sfa_copy_size = cute.size_in_bytes(self.sf_dtype, sfa_smem_layout)
                self.num_tma_load_bytes += sfa_copy_size
    ...
    def kernel(
        ...
        if const_expr(self.is_persistent):
            sched_pipeline = self.make_sched_pipeline(
                self.cluster_shape_mnk, has_C=has_epi_load or const_expr(self.sfa_linear)
            )
        ...
        sfa_panel_pipeline = None
        if const_expr(self.sfa_hoist):
            sfa_panel_pipeline = self.make_sfa_panel_pipeline(cluster_layout_vmnk)
        sfa_ring_pipeline = None
        if const_expr(self.sfa_ring_on):
            sfa_ring_pipeline = self.make_sfa_panel_pipeline(
                cluster_layout_vmnk, num_stages=self.sfa_ring_slots
            )
        ...
        if (
            ...
            # Multicast gather4 (cluster_n > 1): the mask group's rank-0 CTA
            # issues the gathers; peers arm the same tx count and receive the
            # rows via .multicast::cluster. Computed once per kernel from the
            # cluster-layout ARGUMENT (setup-time layouts are region-illegal).
            a_gather_mcast_mask, a_gather_is_issuer = None, None
            if const_expr(self.use_tma_gather and self.num_mcast_ctas_a > 1):
                cta_coord_vmnk = cluster_layout_vmnk.get_flat_coord(cta_rank_in_cluster)
                a_gather_mcast_mask = cute.make_layout_image_mask(
                    cluster_layout_vmnk, cta_coord_vmnk, mode=2
                )
                a_gather_is_issuer = Boolean(cta_coord_vmnk[2] == 0)
            ...
            while work_tile.is_valid_tile:
                ...
                if const_expr(not self.gather_A):
                    ...
                    copy_A, prefetch_A = self._make_gather_A_copy(
                        ...
                        batch_idx,
                        warp_idx,
                        a_gather_mcast_mask=a_gather_mcast_mask,
                        a_gather_is_issuer=a_gather_is_issuer,
                    )
                    if const_expr(varlen_m):
                ...
                if const_expr(self.blockscaled):
                    if const_expr(self.sfa_linear):
                        copy_SFA = None  # gathered by the dedicated SF producer warp
                    else:
        ...
        if warp_idx == self.epi_load_warp_id:
            ...
            if const_expr(self.use_pdl and (has_epi_load or self.sfa_linear)):
                cute.arch.griddepcontrol_wait()
            if const_expr(self.sfa_linear):
                # Dedicated SF producer warp. Measured motivation (B200, nvfp4,
                # S1 d=2048 tile 128x256): the 4 gather warps' A row gather just
                # fits under the fp4-rate MMA (gather-noSF == pre-permuted
                # baseline time), so issuing the SF gather from the same warps
                # put the producers past the MMA budget and cost 12-25% kernel
                # time. This otherwise-idle warp (no C operand) issues the SF
                # cp.asyncs in parallel: it joins the ab pipeline as a producer
                # (32 extra lane arrives), consumes the same prefetched row
                # indices, and walks the same stage sequence as the load warps.
                #
                # sfa_hoist (Q28) changes WHEN, not who: the warp fills one
                # per-work-tile panel holding every k-tile's SF atoms up front,
                # behind its own 1-stage barrier, so an AB stage acquisition no
                # longer waits on 16 scattered 4-B cp.asyncs completing inside
                # the k-loop.
                #
                # sfa_xpose (Q29) changes HOW, inside the unchanged ring: the
                # 16 scattered 4-B cp.asyncs become 4 LDG.128 + a register
                # transpose + 4 conflict-free STS.128, and (mode 2) a k-tile's
                # global read is issued before the acquire of the stage it
                # feeds. The stage barrier then tracks a plain store, so the
                # commit becomes producer_sts_commit (fence + per-lane arrive,
                # same arrive count).
                #
                # sfa_ring (Q30) keeps the production cp.async fill verbatim and
                # changes only WHICH barrier tracks it: a private N-slot ring
                # instead of the AB stage. The fill for k-tile t may start as
                # soon as slot (t mod N) has been UTCCP-consumed - i.e. tiles
                # ahead of the k-tile that needs it - so neither the issue nor
                # the completion sits on the AB stage's ready path.
                tile_scheduler = TileSchedulerCls()
                work_tile = tile_scheduler.initial_work_tile_info()
                sf_prefetch_consumer_state = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer, self.a_prefetch_stage
                )
                if const_expr(self.sfa_hoist):
                    sf_panel_producer_state = pipeline.make_pipeline_state(
                        pipeline.PipelineUserType.Producer, 1
                    )
                elif const_expr(self.sfa_ring_on):
                    sf_ring_producer_state = pipeline.make_pipeline_state(
                        pipeline.PipelineUserType.Producer, self.sfa_ring_slots
                    )
                else:
                ...
                while work_tile.is_valid_tile:
                    tile_coord_mnkl = work_tile.tile_idx
                    batch_idx = tile_coord_mnkl[3]
                    a_prefetch_pipeline.consumer_wait(sf_prefetch_consumer_state)
                    sAIdx_sf = sAIdx[None, sf_prefetch_consumer_state.index]
                    if const_expr(self.sfa_hoist and self.sfa_hoist_pair):
                        copy_SFA_fn = self._make_hoisted_SFA_copy(
                            mSFA_mkl,
                            sSFA,
                            sAIdx_sf,
                            varlen_manager,
                    ...
                    elif const_expr(self.sfa_xpose):
                        load_SFA_fn, store_SFA_fn = self._make_xpose_SFA_copy(
                            mSFA_mkl,
                            sSFA,
                            sAIdx_sf,
                            varlen_manager,
                        ...
                        copy_SFA_fn = self._make_gather_SFA_copy(
                            mSFA_mkl,
                            sSFA,
                            sAIdx_sf,
                            varlen_manager,
                    ...
                    a_prefetch_pipeline.consumer_release(sf_prefetch_consumer_state)
                    sf_prefetch_consumer_state.advance()
                    if const_expr(self.sfa_hoist):
                        # One acquire / one commit per work tile, on the panel's
                        # own barrier (this warp is no longer an AB producer).
                        # The panel slot count IS the k-tile count (host-checked),
                        # so the fill is a static loop and needs no k_tile_cnt.
                        sfa_panel_pipeline.producer_acquire(sf_panel_producer_state)
                        if const_expr(not self.debug_skip_sfa_gather):
                            if const_expr(self.sfa_hoist_pair):
                                copy_SFA_fn()
                            else:
                                for k_tile in cutlass.range_constexpr(self.sfa_slots):
                                    copy_SFA_fn(k_tile, k_tile)
                        sfa_panel_pipeline.producer_cpasync_commit(sf_panel_producer_state)
                        sf_panel_producer_state.advance()
                    elif const_expr(self.sfa_ring_on):
                        # One acquire / one fill / one commit per k-tile, on the
                        # ring's own barrier. Same fill, same bytes, same slot
                        # bytes as production - only the barrier the completion
                        # lands on (and therefore how far ahead it may run)
                        # differs. The ring index is the barrier stage index, so
                        # producer and consumer stay in lockstep by walking the
                        # same (work tile, k-tile) sequence.
                        len_k = varlen_manager.len_k(batch_idx)
                        k_tile_cnt = cute.ceil_div(len_k, self.cta_tile_shape_mnk[2])
                        peek_sf_empty = Boolean(True)
                        if 0 < k_tile_cnt:
                            peek_sf_empty = sfa_ring_pipeline.producer_try_acquire(
                                sf_ring_producer_state
                            )
                        for k_tile in cutlass.range(k_tile_cnt, unroll=1):
                            sfa_ring_pipeline.producer_acquire(
                                sf_ring_producer_state, peek_sf_empty
                            )
                            # The skip arm keeps the acquire/commit and drops
                            # only the copies (arm d's structure, unchanged).
                            if const_expr(not self.debug_skip_sfa_gather):
                                copy_SFA_fn(k_tile, sf_ring_producer_state.index)
                            sfa_ring_pipeline.producer_cpasync_commit(sf_ring_producer_state)
                            sf_ring_producer_state.advance()
                            peek_sf_empty = Boolean(True)
                        ...
                        len_k = varlen_manager.len_k(batch_idx)
                        k_tile_cnt = cute.ceil_div(len_k, self.cta_tile_shape_mnk[2])
                        skip = const_expr(self.debug_skip_sfa_gather)
                        if const_expr(self.sfa_xpose == 2 and not skip):
                            # Lookahead-1 prologue: the first k-tile's global
                            # read is issued before the first stage acquire.
                            if 0 < k_tile_cnt:
                                load_SFA_fn(Int32(0))
                        peek_sf_empty = Boolean(True)
                        if 0 < k_tile_cnt:
                            peek_sf_empty = ab_pipeline.producer_try_acquire(sf_producer_state)
                        for k_tile in cutlass.range(k_tile_cnt, unroll=1):
                            if const_expr(self.sfa_xpose == 1 and not skip):
                                # In-place: the read sits on the acquire ->
                                # arrive path, exactly where the cp.async did.
                                load_SFA_fn(k_tile)
                            ab_pipeline.producer_acquire(
                                sf_producer_state, peek_sf_empty, Boolean(False)
                            )
                            if const_expr(not skip):
                                store_SFA_fn(sf_producer_state.index)
                            # Plain stores, so the barrier cannot track a copy:
                            # fence into the async proxy, then arrive per lane
                            # (same count as cp_async_mbarrier_arrive_noinc).
                            ab_pipeline.producer_sts_commit(sf_producer_state)
                            if const_expr(self.sfa_xpose == 2 and not skip):
                                # Single register set: the next k-tile's read
                                # can only issue once this one has been stored,
                                # but it still lands before the NEXT acquire.
                                if k_tile + 1 < k_tile_cnt:
                                    load_SFA_fn(k_tile + 1)
                        ...
                        for k_tile in cutlass.range(k_tile_cnt, unroll=1):
                            ...
                            # The skip arm keeps the acquire/commit (the AB barrier
                            # counts this warp's lanes) and drops only the copies.
                            if const_expr(not self.debug_skip_sfa_gather):
                                copy_SFA_fn(k_tile, sf_producer_state.index)
                            ab_pipeline.producer_cpasync_commit(sf_producer_state)
                            sf_producer_state.advance()
                            peek_sf_empty = Boolean(True)
                            if k_tile + 1 < k_tile_cnt:
                                peek_sf_empty = ab_pipeline.producer_try_acquire(sf_producer_state)
        ...
        if warp_idx == self.mma_warp_id:
            ...
            if const_expr(self.sfa_hoist):
                sfa_panel_consumer_state = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer, 1
                )
            sfa_ring_consumer_state = None
            if const_expr(self.sfa_ring_on):
                sfa_ring_consumer_state = pipeline.make_pipeline_state(
                    pipeline.PipelineUserType.Consumer, self.sfa_ring_slots
                )
            while work_tile.is_valid_tile:
                # Get tile coord from tile scheduler
                # (pid_m, pid_n, split_idx | None, batch_idx), decoded by the scheduler
                ...
                iket.range_push("mma")
                if const_expr(self.sfa_hoist):
                    # The whole tile's SF arrives on ONE barrier, before the first
                    # UTCCP. Waiting here (rather than inside the k-loop) is the
                    # point of the panel: AB stages then wait on TMA only.
                    sfa_panel_pipeline.consumer_wait(sfa_panel_consumer_state)
                (
                    ab_consumer_state,
                    acc_producer_state,
                    tiled_mma,
                    sfa_ring_consumer_state,
                ) = self.mma(
                    ab_pipeline,
                    acc_pipeline,
                    ab_consumer_state,
                ...
                if const_expr(self.sfa_hoist):
                    # Release AFTER the tile's last copy_s2t_sfa. The arrive is a
                    # tcgen05 commit, so it fires only once this warp's async
                    # UTCCPs have drained - the panel cannot be refilled under a
                    # still-reading tcgen05.cp.
                    sfa_panel_pipeline.consumer_release(sfa_panel_consumer_state)
                    sfa_panel_consumer_state.advance()
                iket.range_pop()
                # Advance to next tile
                tile_scheduler.advance_to_next_work()
                work_tile = tile_scheduler.get_current_work()
    ...
    def _make_gather_A_copy(
        ...
        batch_idx: Int32,
        warp_idx: Int32,
        a_gather_mcast_mask=None,  # Int16 mask (multicast gather4, cluster_n > 1)
        a_gather_is_issuer=None,  # Boolean: this CTA issues the gather4s
    ):
        ...
        if const_expr(self.use_tma_gather):
            if const_expr(varlen_m):
                copy_A = copy_utils.gather_m_get_tma_copy_fn(
                    ...
                    num_cta=num_cta,
                    multicast_mask=a_gather_mcast_mask,
                    is_issuer=a_gather_is_issuer,
                )
            elif const_expr(varlen_k):
    ...
    @cute.jit
    def _make_gather_SFA_copy(
        self,
        mSFA_w: cute.Tensor,  # (T, K/(4*sf_vec)) Int32 gmem: the linear SF viewed as words
        sSFA: cute.Tensor,  # blocked SFA smem (make_smem_layout_sfa), staged
        sAIdx: cute.Tensor,  # (tile_M,) Int32 smem, current prefetch stage slice
        varlen_manager: VarlenManager,
        tile_coord_mnkl,
        batch_idx: Int32,
    ):
        """cp.async row-gather of LINEAR scale factors into the blocked smem atom.

        Why this works without a rearrangement pass: within the 512-B SF atom
        ((32, 4), (vec, 4)) : ((16, 4), (0, 1)), the 4 sf-k bytes of a fixed row
        m are contiguous at byte (m%32)*16 + (m//32)*4 — one aligned Int32 word.
        A row's SF for one k-tile is therefore rk_words words that are contiguous
        in the linear gmem layout and land at 512-B-strided smem words. One 4-B
        cp.async per (row, k-atom); 128 rows x rk_words words per k-tile, issued
        by the dedicated SF producer warp (32 lanes). The word view requires
        K % 64 == 0 (asserted host-side), and K % mma_tiler_k == 0 removes the
        k-tail predicate.

        Row indices are the same global rows the A gather uses (sAIdx); they are
        read into registers HERE, at construction time, inside the caller's
        a_prefetch consume window. OOB rows load row 0 — finite scales, and the
        epilogue masks those accumulator rows, exactly like the A gather.
        """
        tile_m = self.cta_tile_shape_mnk[0]
        assert tile_m == 128, "sfa_linear gather requires a 128-row CTA tile"
        rk_words = self.mma_tiler[2] // (self.sf_vec_size * 4)
        stage_words = 128 * rk_words
        # (The word-view <-> blocked-layout size fence lives in __call__ —
        # layout algebra on setup-time layouts is illegal inside the kernel's
        # staged control-flow region.)
        #
        # Lean addressing: everything row-dependent (gmem word offset of the
        # gathered row, smem word offset of its atom slot) is computed ONCE per
        # work tile into registers; the per-k-tile hot path is two adds and one
        # 4-byte cp.async per (row, k-atom). The first cut of this loader went
        # through ThrCopy partitions + per-call layout slicing and cost ~33
        # warp-instructions per k-tile (25% of kernel time at tile_n=128,
        # measured with smsp__inst_executed deltas on B200).
        num_threads = cute.arch.WARP_SIZE  # one dedicated producer warp
        passes = const_expr(tile_m * rk_words // num_threads)
        rows_per_pass = const_expr(num_threads // rk_words)
        dma_tidx = cute.arch.thread_idx()[0] - self.epi_load_warp_id * cute.arch.WARP_SIZE
        col = dma_tidx % rk_words
        row0 = dma_tidx // rk_words
        len_m = varlen_manager.len_m(batch_idx)
        limit_m = len_m - tile_coord_mnkl[0] * tile_m
        row_stride_w = mSFA_w.stride[0]
        src_off = cute.make_rmem_tensor(passes, Int32)
        dst_off = cute.make_rmem_tensor(passes, Int32)
        for i in cutlass.range(passes, unroll_full=True):
            row = row0 + i * rows_per_pass
            # smem atom slot of row m: word (m%32)*4 + m//32 (+ col*128)
            dst_off[i] = (row % 32) * 4 + row // 32 + col * 128
            if row < limit_m:
                src_off[i] = Int32(sAIdx[row] * row_stride_w)
            else:
                src_off[i] = 0  # row 0's scales are finite; the row is masked later
        atom_sf = cute.make_copy_atom(cpasync.CopyG2SOp(), Int32, num_bits_per_copy=32)
        sSFA_w_ptr = cute.recast_ptr(sSFA.iterator, dtype=Int32)
        src_ptr = mSFA_w.iterator
        lay1 = cute.make_layout(1)

        def copy_fn(src_idx, dst_idx):
            k_off = src_idx * rk_words + col
            st_off = dst_idx * stage_words
            for i in cutlass.range_constexpr(passes):
                src = cute.make_tensor(src_ptr + (src_off[i] + k_off), lay1)
                dst = cute.make_tensor(sSFA_w_ptr + (st_off + dst_off[i]), lay1)
                cute.copy(atom_sf, src, dst)

        return copy_fn

    @cute.jit
    def _make_xpose_SFA_copy(
        self,
        mSFA_w: cute.Tensor,  # (T, K/(4*sf_vec)) Int32 gmem: the linear SF as words
        sSFA: cute.Tensor,  # blocked SFA smem (make_smem_layout_sfa), staged
        sAIdx: cute.Tensor,  # (tile_M,) Int32 smem, current prefetch stage slice
        varlen_manager: VarlenManager,
        tile_coord_mnkl,
        ...
        """LDG.128 -> register transpose -> STS.128 fill of one AB stage's SFA.

        Byte-for-byte the same fill as ``_make_gather_SFA_copy`` — same stage,
        same 512 destination words, same OOB-row-0 convention — with a
        different instruction shape on both sides of the L1TEX pipe:

        * **load.** Lane ``l`` owns rows ``l, l+32, l+64, l+96``. A row's
          ``rk_words = 4`` scale words for one k-tile are CONTIGUOUS in the
          linear gmem layout, so each row is one 16-B LDG: 4 per k-tile instead
          of 16 four-byte cp.asyncs, and each is a single aligned 16-B run
          instead of a scattered 4-B request. (Alignment: the linear row stride
          is ``K/64`` words, a multiple of 4 whenever ``K % 256 == 0`` — the
          host constraint for this path — and ``k_tile * 4`` is a multiple of 4
          by construction.)

        * **transpose.** No shuffle is needed. The destination word of row m,
          k-word c is ``(m%32)*4 + m//32 + c*128``; substituting ``m = 32*i + l``
          gives ``4*l + i + 128*c``, so for a fixed c the four rows lane l
          already holds land at four CONSECUTIVE words ``4l .. 4l+3``. The
          transpose is a register re-index inside the lane.

        * **store.** One 16-B STS per k-word: 32 lanes x 16 B covering the
          512 contiguous bytes of one k-atom, i.e. 4 conflict-free 128-B
          wavefronts per instruction. The 4-B scattered cp.async it replaces
          measured 28.2 shared-bank conflicts PER INSTRUCTION (Q28 verifier
          S4), ~4,080 serialized shared wavefronts per work tile.

        Returns ``(load_fn, store_fn)`` rather than one copy fn: splitting them
        is the point of arm 2, which issues a k-tile's global read before
        acquiring the stage it will be stored into, so the read latency is not
        on the acquire -> arrive path. The registers are a single set (16 Int32
        for the data, 4 for the row bases): ``store_fn`` consumes them before
        the next ``load_fn`` overwrites them.

        ``sfa_xpose_scatter`` builds the FALSIFICATION CONTROL instead: the
        same split load/store and the same barrier rewiring, but with the
        cp.async loader's exact thread mapping (lane t owns k-word t%4 of the
        8-row group t//4, 16 words apiece), so both sides of the pipe keep
        production's scattered access structure — 16 four-byte loads and 16
        four-byte stores per k-tile. It answers "is any delta the vectorized
        conflict-free store, or merely leaving cp.async?". Note the two changes
        cannot be separated the other way round: with the transposed ownership
        a lane's four destination words are CONSECUTIVE by construction, so
        ptxas re-merges any attempt to issue them as separate 4-B stores
        (verified in SASS) — the transpose and the vectorization are the same
        fact.

        The stores are generic-proxy writes and UTCCP reads the stage through
        the async proxy, so the caller must commit with
        ``PipelineTmaCpAsyncUmma.producer_sts_commit`` (fence + per-lane arrive)
        instead of ``producer_cpasync_commit``; the arrive count is identical.
        """
        tile_m = self.cta_tile_shape_mnk[0]
        assert tile_m == 128, "sfa_linear gather requires a 128-row CTA tile"
        rk_words = const_expr(self.mma_tiler[2] // (self.sf_vec_size * 4))
        assert rk_words == 4, "the register transpose is derived for rk_words == 4"
        stage_words = const_expr(tile_m * rk_words)
        num_threads = cute.arch.WARP_SIZE  # one dedicated producer warp
        lane = cute.arch.thread_idx()[0] - self.epi_load_warp_id * num_threads
        len_m = varlen_manager.len_m(batch_idx)
        limit_m = len_m - tile_coord_mnkl[0] * tile_m
        row_stride_w = mSFA_w.stride[0]
        sSFA_w_ptr = cute.recast_ptr(sSFA.iterator, dtype=Int32)
        ...
        if const_expr(self.sfa_xpose_scatter):
            # Control arm: the cp.async loader's own thread mapping and word
            # addresses, issued as separate 4-B LDG + 4-B STS. The addresses
            # of one lane's consecutive words are 32 apart (rows step by 8), so
            # nothing here is mergeable and the scattered structure survives.
            passes = const_expr(tile_m * rk_words // num_threads)  # 16
            rows_per_pass = const_expr(num_threads // rk_words)  # 8
            col = lane % rk_words
            row0 = lane // rk_words
            src_off = cute.make_rmem_tensor(passes, Int32)
            dst_off = cute.make_rmem_tensor(passes, Int32)
            for i in cutlass.range(passes, unroll_full=True):
                ...
                if row < limit_m:
                    ...
                    src_off[i] = 0  # row 0's scales are finite; masked later
            src_ptr = mSFA_w.iterator
            regs = cute.make_rmem_tensor(passes, Int32)

            def load_fn(k_tile):
                k_off = k_tile * rk_words + col
                for i in cutlass.range_constexpr(passes):
                    regs[i] = cute.make_tensor(src_ptr + (src_off[i] + k_off), lay1)[0]

            def store_fn(dst_idx):
                st_off = dst_idx * stage_words
                for i in cutlass.range_constexpr(passes):
                    cute.make_tensor(sSFA_w_ptr + (st_off + dst_off[i]), lay1)[0] = regs[i]

            return load_fn, store_fn

        rows_per_lane = const_expr(tile_m // num_threads)  # 4: l, l+32, l+64, l+96
        src_off = cute.make_rmem_tensor(rows_per_lane, Int32)
        for i in cutlass.range(rows_per_lane, unroll_full=True):
            row = i * num_threads + lane
            if row < limit_m:
                src_off[i] = Int32(sAIdx[row] * row_stride_w)
            else:
                src_off[i] = 0  # row 0's scales are finite; the row is masked later
        # The fake linear-SF operand is declared 4-B aligned (its row stride is
        # dynamic); the 16-B loads need the real contract, which the host
        # asserts for this path (validate_blockscaled_sf: 16-B base, row stride
        # a multiple of 4 words).
        src_ptr = cute.make_ptr(
            Int32, mSFA_w.iterator.toint(), cute.AddressSpace.gmem, assumed_align=16
        )
        atom_ld = cute.make_copy_atom(cute.nvgpu.CopyUniversalOp(), Int32, num_bits_per_copy=128)
        atom_st = cute.make_copy_atom(cute.nvgpu.CopyUniversalOp(), Int32, num_bits_per_copy=128)
        lay4 = cute.make_layout(rk_words)
        # (k_word, row group), mode 0 contiguous: regs[None, i] is the 4-word
        # k-run of one row (an LDG destination) and regs[c, i] is what the STS
        # of k-word c wants — the transpose, for free, at trace time.
        regs = cute.make_rmem_tensor((rk_words, rows_per_lane), Int32)

        def load_fn(k_tile):
            for i in cutlass.range_constexpr(rows_per_lane):
                off = cute.assume(src_off[i] + k_tile * rk_words, divby=rk_words)
                cute.copy(atom_ld, cute.make_tensor(src_ptr + off, lay4), regs[None, i])

        def store_fn(dst_idx):
            st_off = dst_idx * stage_words
            for c in cutlass.range_constexpr(rk_words):
                # One word per row per k-atom => tile_m words per k-atom (512 B),
                # and lane l owns the 4 consecutive words 4l..4l+3 of each.
                base = st_off + c * tile_m + lane * rows_per_lane
                st = cute.make_rmem_tensor(rows_per_lane, Int32)
                for i in cutlass.range_constexpr(rows_per_lane):
                    st[i] = regs[c, i]
                dst = cute.make_tensor(sSFA_w_ptr + cute.assume(base, divby=rows_per_lane), lay4)
                cute.copy(atom_st, st, dst)

        return load_fn, store_fn

    @cute.jit
    def _make_hoisted_SFA_copy(
        self,
        mSFA_w: cute.Tensor,  # (T, K/(4*sf_vec)) Int32 gmem: the linear SF as words
        sSFA: cute.Tensor,  # the hoisted SF panel (one slot per k-tile)
        sAIdx: cute.Tensor,  # (tile_M,) Int32 smem, current prefetch stage slice
        varlen_manager: VarlenManager,
        tile_coord_mnkl,
        batch_idx: Int32,
    ):
        """Row-major fill of the hoisted SF panel: one warp instruction per row.

        Same bytes, same destinations as ``_make_gather_SFA_copy`` looped over
        k-tiles — only the (lane -> word) assignment differs, and it is chosen
        for the GLOBAL side. Across the whole panel a row's scales are
        ``row_words = K / (4*sf_vec)`` CONTIGUOUS gmem words, so handing one row
        to the 32 lanes makes each instruction read 32 x 4 B = 128 contiguous
        bytes: 4 fully-used 32-B sectors. The per-stage loader instead reads 4
        words per row for 8 rows, i.e. 8 sectors that carry 16 useful bytes
        each, and re-fetches the other half of every sector on the next k-tile —
        1024 vs 512 SF sector-wavefronts per work tile (Q1 §4: 1.69M sectors for
        25 MB of useful SF).

        Panel word of (row m, word w), unfolding slot = w // rk_words and
        k-atom = w % rk_words against the 512-word slot stride:
            w * 128 + (m%32) * 4 + m//32
        (the k-atoms of a row are one word each, 512 B apart in the blocked
        atom sequence; see _make_gather_SFA_copy for the atom algebra).

        Row indices are broadcast from registers, not re-read from smem: lane l
        caches rows l, l+32, l+64, l+96 inside the caller's a_prefetch consume
        window, and each instruction shuffles the owning lane's value. OOB rows
        read row 0, exactly like the A gather (the epilogue masks them).
        """
        tile_m = self.cta_tile_shape_mnk[0]
        assert tile_m == 128, "sfa_linear gather requires a 128-row CTA tile"
        num_threads = cute.arch.WARP_SIZE  # one dedicated producer warp
        row_words = const_expr(self.sfa_panel_row_words)
        runs_per_row = const_expr(row_words // num_threads)  # row_words % 32 == 0, asserted
        rows_per_lane = const_expr(tile_m // num_threads)
        lane = cute.arch.thread_idx()[0] - self.epi_load_warp_id * num_threads
        len_m = varlen_manager.len_m(batch_idx)
        limit_m = len_m - tile_coord_mnkl[0] * tile_m
        row_stride_w = mSFA_w.stride[0]
        src_base = cute.make_rmem_tensor(rows_per_lane, Int32)
        for i in cutlass.range(rows_per_lane, unroll_full=True):
            row = i * num_threads + lane
            if row < limit_m:
                src_base[i] = Int32(sAIdx[row] * row_stride_w)
            else:
                src_base[i] = 0  # row 0's scales are finite; the row is masked later
        atom_sf = cute.make_copy_atom(cpasync.CopyG2SOp(), Int32, num_bits_per_copy=32)
        sSFA_w_ptr = cute.recast_ptr(sSFA.iterator, dtype=Int32)
        src_ptr = mSFA_w.iterator
        lay1 = cute.make_layout(1)
        ...
        def copy_fn():
            for i in cutlass.range_constexpr(rows_per_lane):
                for l in cutlass.range_constexpr(num_threads):
                    row = i * num_threads + l
                    base = cute.arch.shuffle_sync(src_base[i], offset=l)
                    dst_row = const_expr((row % 32) * 4 + row // 32)
                    for j in cutlass.range_constexpr(runs_per_row):
                        w = j * num_threads + lane
                        src = cute.make_tensor(src_ptr + (base + w), lay1)
                        dst = cute.make_tensor(sSFA_w_ptr + (w * 128 + dst_row), lay1)
                        cute.copy(atom_sf, src, dst)
    ...
    def load_AB_gather_A(
        ...
        # TMA load on B (and SFB) and cp.async on A (and linear SFA)
        for k_tile in cutlass.range(k_tile_cnt - 1, unroll=2 if const_expr(varlen_m) else 1):
            ...
            if const_expr(copy_SFA is not None and not self.debug_skip_sfa_gather):
                # cp.async issued by the same lanes as A: producer_cpasync_commit
                # below tracks these transfers too.
                copy_SFA(k_tile, smem_idx)
        ...
        if 0 < k_tile_cnt:
            ...
            if const_expr(copy_SFA is not None and not self.debug_skip_sfa_gather):
                # K % mma_tiler_k == 0 is asserted host-side for the linear-SFA
                # path, so the last k-tile's SF window is always full: no k
                # predication needed here.
                copy_SFA(k_tile, smem_idx)
            ab_pipeline.producer_cpasync_commit(ab_producer_state)
            ab_producer_state.advance()
    ...
    def mma(
        ...
        sfa_ring_pipeline: Optional[cutlass.pipeline.PipelineAsync] = None,
        sfa_ring_consumer_state: Optional[cutlass.pipeline.PipelineState] = None,
    ) -> Tuple[
        ...
        for k_tile in cutlass.range(k_tile_cnt, unroll=1):
            ...
            if is_leader_cta:
                ...
                if const_expr(blockscaled):
                    # Hoisted SFA: the panel is indexed by the ABSOLUTE k-tile
                    # (one slot per k-tile, filled before the loop), not by the
                    # AB stage. Ringed SFA: indexed by its own ring slot, whose
                    # full/empty barrier is separate from the AB stage's, so the
                    # wait and the release are per k-tile here. SFB still rides
                    # the AB stages either way.
                    if const_expr(self.sfa_hoist):
                        copy_s2t_sfa(k_tile)
                    elif const_expr(self.sfa_ring_on):
                        sfa_ring_pipeline.consumer_wait(sfa_ring_consumer_state)
                        copy_s2t_sfa(sfa_ring_consumer_state.index)
                        # Release right after the UTCCP is ISSUED: the arrive is
                        # a tcgen05 commit, so it fires only once this warp's
                        # async tcgen05.cp has drained and the slot cannot be
                        # refilled under a still-reading copy. (The MMAs below
                        # read SFA from TMEM, not from this slot.) The state
                        # advance is a loop-carried mutation, so it lives
                        # outside this dynamic if, next to ab_consumer_state's.
                        sfa_ring_pipeline.consumer_release(sfa_ring_consumer_state)
                    else:
            ...
            if const_expr(sfa_ring_consumer_state is not None):
                sfa_ring_consumer_state.advance()
        ...
        return ab_consumer_state, acc_producer_state, tiled_mma, sfa_ring_consumer_state
    ...
    def make_ab_pipeline(
        ...
        # The SF producer warp joins this pipeline only when SFA lives in the AB
        # stages; the hoisted panel and the SF ring have their own barrier
        # (make_sfa_panel_pipeline).
        sf_in_ab = const_expr(self.sfa_linear and not self.sfa_hoist and not self.sfa_ring_on)
        if const_expr(not self.gather_A or self.use_tma_gather):
            producer_cnt = 1
            if const_expr(sf_in_ab):
                # TMA-gather A + cp.async SF: the dedicated SF producer warp's
                # lanes arrive at the same full barrier.
                producer_cnt += cute.arch.WARP_SIZE
        else:
            producer_cnt = self.num_ab_load_warps * cute.arch.WARP_SIZE
            if const_expr(sf_in_ab):
                # The dedicated SF producer warp's lanes also cp.async into the
                # stage and arrive at the same full barrier.
                producer_cnt += cute.arch.WARP_SIZE
        ...
        pipeline_checks.check_arrive_count(
            ...
            pipeline_checks.tma_producer_arrives(
                ...
                + (1 if const_expr(sf_in_ab) else 0),
            ),
        ...
        if const_expr((not self.gather_A or self.use_tma_gather) and not self.sfa_linear):
    ...
    @cute.jit
    def make_sfa_panel_pipeline(
        self, cluster_layout_vmnk: cute.Layout, num_stages: int = 1
    ) -> pipeline.PipelineAsync:
        """Pipeline guarding an SFA buffer that lives outside the AB stages.

        One stage = the hoisted per-work-tile panel (Q28); ``num_stages = N`` =
        the SF ring (Q30), one barrier stage per ring slot, so the SF warp may
        run N k-tiles ahead of the UTCCP instead of being capped by ab_stage.

        Producer = the dedicated SF warp's 32 lanes (cp.async completion arrives);
        consumer = the mma warp, whose release must be a tcgen05 commit because
        the buffer is read by ASYNC tcgen05.cp (UTCCP) — the same mechanism the AB
        pipeline uses before it recycles an SFA stage. A plain mbarrier arrive
        here would let a later fill overwrite scales a still-running tcgen05.cp
        has not finished reading.
        """
        producer_cnt = cute.arch.WARP_SIZE
        pipeline_checks.check_arrive_count(
            "sm100 sfa_panel_pipeline.producer",
            producer_cnt,
            pipeline_checks.tma_producer_arrives(num_tma_warps=0, cpasync_warps=1),
            num_sf_warps=1,
        )
        consumer_cnt = 1  # one tcgen05.commit from the mma warp
        pipeline_checks.check_arrive_count(
            "sm100 sfa_panel_pipeline.consumer",
            consumer_cnt,
            pipeline_checks.umma_producer_arrives(),
        )
        return PipelineAsyncUmma.create(
            num_stages=num_stages,
            producer_group=pipeline.CooperativeGroup(pipeline.Agent.Thread, producer_cnt),
            consumer_group=pipeline.CooperativeGroup(pipeline.Agent.Thread, consumer_cnt),
            cta_layout_vmnk=cluster_layout_vmnk,
            defer_sync=True,
        )
    ...
    def make_a_prefetch_pipeline(self) -> pipeline.PipelineAsync:
        ...
        consumer_arrive_cnt = self.num_ab_load_warps + (1 if self.sfa_linear else 0)
        # One arrive per AB-load consumer warp (elected lane); the dedicated SF
        # producer warp also reads the indices, adding one more.
        elect_one_release = True
        pipeline_checks.check_arrive_count(
            "sm100 a_prefetch_pipeline.consumer",
            consumer_arrive_cnt,
            pipeline_checks.async_thread_arrives(
                self.num_ab_load_warps + (1 if self.sfa_linear else 0),
                per_warp=elect_one_release,
            ),
            num_ab_load_warps=self.num_ab_load_warps,
        )
    ...
    def _compute_stages(
        ...
        # (ab, epi, epi_c) stage-count overrides, None entries keep the
        # heuristic. Overriding epi pins it (no leftover top-up); overriding
        # ab asserts the total still fits smem. Stage counts change pipeline
        # depth only, never the K reduction order - outputs stay bitwise.
        overrides: Tuple[Optional[int], Optional[int], Optional[int]] | None = None,
        # SF panel (Q28): >0 takes SFA out of the per-AB-stage budget and books
        # it as one fixed buffer of this many k-tile slots instead.
        sfa_panel_slots: int = 0,
    ) -> Tuple[int, int, int]:
        ...
        ov_ab, ov_epi, ov_epi_c = overrides if overrides is not None else (None, None, None)
        # Default D stages
        epi_stage = 4 if cute.size(epi_tile[1]) <= 16 else 2
        if ov_epi is not None:
            epi_stage = ov_epi
        ...
        if ov_epi_c is not None and epi_c_stage > 0:
            epi_c_stage = ov_epi_c
        ...
        sfa_panel_bytes = 0
        if const_expr(blockscaled):
            sfa_bytes_one = cute.size_in_bytes(sf_dtype, sfa_smem_layout_staged_one)
            if const_expr(sfa_panel_slots > 0):
                # Hoisted: one buffer for the whole work tile, not one per AB stage.
                sfa_panel_bytes = sfa_bytes_one * sfa_panel_slots
            else:
                ab_bytes_per_stage += sfa_bytes_one
            ab_bytes_per_stage += cute.size_in_bytes(sf_dtype, sfb_smem_layout_staged_one)
        ...
        remaining_bytes = (
            smem_capacity // occupancy - mbar_helpers_bytes - epi_bytes - sfa_panel_bytes
        )
        ab_stage = remaining_bytes // ab_bytes_per_stage
        if ov_ab is not None:
            assert ov_ab * ab_bytes_per_stage <= remaining_bytes, (
                f"ab_stage override {ov_ab} does not fit smem "
                f"({remaining_bytes // ab_bytes_per_stage} max at this epi budget)"
            )
            ab_stage = ov_ab

        # Refine epilogue stages:
        # Calculate remaining smem after allocating for A/B stages and reserved bytes
        # Add remaining unused smem to epilogue (skipped when epi is pinned)
        if epi_bytes_per_stage > 0 and ov_epi is None:
