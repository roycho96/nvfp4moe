...
def gemm_tuned(
    ...
    if blockscaled:
        # Kernel support exists (quack.gemm.gemm with a LINEAR (T, K/(4*vec))
        # int32 SFA + A_idx; see gemm_sm100.sfa_linear), but BlockScaledOperand
        # carries only blocked scales, so the container path stays closed until
        # the per-operand scale_layout tag (AI/blockscaled_api.md section 9.4)
        # lands.
        assert not gather_A, (
            "Blockscaled gather_A through BlockScaledOperand needs the linear "
            "scale_layout tag; call quack.gemm.gemm with a linear SFA instead"
        )
    ...
    if gather_A:
        ...
        assert config.cluster_n == 1 or config.use_tma_gather, (
