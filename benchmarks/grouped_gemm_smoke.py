"""Correctness and latency smoke tests for the LightMoE grouped GEMM."""
...
from lightmoe.kernels.gated import GatedBackwardEpilogue
...
def run_case(
    ...
    qb, sfb = quantize_expert_stack([b[e] for e in range(experts)], pts_b)
    qa = qa_u8.view(torch.float4_e2m1fn_x2)
    alpha = pts_a * pts_b

    output = torch.empty(total, n, dtype=output_dtype, device=device)
    runtime = grouped_nvfp4_gemm(
        experts,
        n,
        k,
        tile_m,
    ...
    def run_gemm():
        runtime(qa, qb, output, cu, sfa, sfb, alpha)

    run_gemm()
    torch.cuda.synchronize()
    _check_gemm_samples(output, qa_u8, qb, sfa, sfb, cu, pts_a, pts_b)
    if profile_only:
        runtime.prepare(qa, qb, output, cu, sfa, sfb, alpha)
        torch.cuda.nvtx.range_push("lightmoe_profile")
        runtime.launch()
        torch.cuda.nvtx.range_pop()
        torch.cuda.synchronize()
        return None
    lightmoe_ms = _time(run_gemm)
    prepare_ms = _time(lambda: runtime.prepare(qa, qb, output, cu, sfa, sfb, alpha))
    runtime.prepare(qa, qb, output, cu, sfa, sfb, alpha)
    kernel_ms = _time(runtime.launch)
    result = {
        "experts": experts,
        "m": total,
        "n": n,
...
def run_dgrad2_case(
    ...
    qb, sfb = quantize_expert_stack([weight[e] for e in range(experts)], pts_b)
    qa = qa_u8.view(torch.float4_e2m1fn_x2)
    alpha = pts_a * pts_b

    input_gradient = torch.empty(total, 2 * n, dtype=torch.bfloat16, device=device)
    auxiliary = torch.empty(total, n, dtype=torch.bfloat16, device=device)
    runtime = grouped_nvfp4_gemm(
        experts,
        n,
        k,
    ...
    def run_gemm():
        runtime(
            qa,
            qb,
            input_gradient.view(torch.int32),
            cu,
            sfa,
            sfb,
            alpha,
    ...
    first_dh = input_gradient.clone()
    first_aux = auxiliary.clone()
    run_gemm()
    torch.cuda.synchronize()
    if not torch.equal(input_gradient, first_dh) or not torch.equal(auxiliary, first_aux):
        raise AssertionError("LightMoE dgrad2 is not deterministic")
    if profile_only:
        prepare_gemm()
        torch.cuda.nvtx.range_push("lightmoe_profile")
        runtime.launch()
...
def main():
    ...
    parser.add_argument(
        ...
        choices=(
            ...
            "gate-up-deepseek-local",
            "down-deepseek-local",
            "dgrad1-deepseek-local",
            "dense-qwen-down",
            "dense-deepseek-gate-up",
            "dense-deepseek-down",
        ),
    )
    ...
    if args.profile_case == "dgrad2-deepseek-local":
        ...
        return
    if args.profile_case == "gate-up-deepseek-local":
        run_case(8, 4096, 7168, 65_536, 256, 256, profile_only=True)
        return
    if args.profile_case == "down-deepseek-local":
    ...
    if args.profile_case == "dgrad1-deepseek-local":
        ...
        return
    if args.profile_case == "dense-qwen-down":
        run_dense_profile(2048, 768, 8192, 256, 128)
        return
    if args.profile_case == "dense-deepseek-gate-up":
        run_dense_profile(4096, 7168, 8192, 256, 256)
        return
    if args.profile_case == "dense-deepseek-down":
