...
- rewrite_tensor_shape: Tensor shape rewrite utility
"""
...
def store_tma_desc(
    ...
    *,
    ...
    tma_desc_val = _cute_nvgpu_ir.atom_get_value(
        mlir_desc_ty, atom_value, field_attr, loc=loc, ip=ip
    )

    desc_full = builtin.unrealized_conversion_cast(
        [llvm_struct_full_ty], [tma_desc_val], loc=loc, ip=ip
    )
...
def gmem_ptr_to_generic(gmem_ptr: Pointer, *, loc=None, ip=None) -> Pointer:
    ...
    # Cast the LLVM pointer to generic address space.
...
def generic_ptr_to_gmem(generic_ptr: Pointer, *, loc=None, ip=None) -> Pointer:
    ...
    # Cast the LLVM pointer to global address space.
...
class WgradTensorMap:
    """Map an expert work tile to its physical tensors and descriptors."""
    ...
    def __new_from_mlir_values__(self, values):
        return WgradTensorMap(
            tensormap_ctor=new_from_mlir_values(self.tensormap_ctor, values),
    ...
    def get_gmem_tensor(
        ...
        if cutlass.const_expr(tensor_name in ("a", "b")):
            ...
            return rewrite_tensor_shape(real, (shape[0], tokens_i, c1)), None
        ...
        if cutlass.const_expr(tensor_name == "c"):
            if cutlass.const_expr(self.weight_mode == MoEWeightMode.DENSE):
                ...
                return rewrite_tensor_shape(real, (shape[0], shape[1], c1)), None
        ...
        if cutlass.const_expr(tensor_name in ("sfa", "sfb")):
            sf_layout = tile_atom_to_shape_SF((shape[0], tokens_i, c1), self.sf_vec_size)
        ...
        raise ValueError(f"unknown wgrad tensor {tensor_name!r}")
...
class WgradSfTensormapConstructor(OnlineTensormapDescCreator):
    ...
    def __init__(
        ...
        self.workspace = TensormapWorkspace(
            workspace_ptr, self.slot_names(input_order, weight_mode)
        )
    ...
    def construct_and_write(self, expert_idx: Int32, dependency=None) -> None:
        ...
        sfa_layout_i = blockscaled_utils.tile_atom_to_shape_SF(
            per_expert_sfa_shape, self.sf_vec_size
        )
        ...
        sfb_layout_i = blockscaled_utils.tile_atom_to_shape_SF(
            per_expert_sfb_shape, self.sf_vec_size
        )
...
class WgradTensormapConstructor(OnlineTensormapDescCreator):
    ...
    def __init__(
        ...
        self.b_tensor = b_tensor
        self.workspace = TensormapWorkspace(
            workspace_ptr, self.slot_names(self.input_order, weight_mode)
        )
