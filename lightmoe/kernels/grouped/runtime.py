"""Host runtime for the LightMoE grouped NVFP4 GEMM kernel."""
...
class GroupedNvfp4Gemm:
    """Grouped NVFP4 GEMM for contiguous expert-major assignments."""
    ...
    def __init__(
        ...
        if experts > 256:
            raise ValueError("grouped GEMM supports at most 256 local experts")
        if k % 64:
            raise ValueError("grouped GEMM requires K aligned to 64")
        if tile_m not in (128, 256) or tile_n < 8 or tile_n > 256 or tile_n % 8:
            raise ValueError("grouped GEMM supports M=128/256 and N=8..256 aligned to 8")
    ...
    def prepare(
        ...
        if self.gather_b:
            ...
            assignment_count = gather_idx.numel()
        ...
        if tuple(out.shape) != (assignment_count, output_cols) or out.dtype != self.output_dtype:
            raise ValueError(
                f"output must use {self.output_dtype} with shape "
                f"({assignment_count}, {output_cols})"
            )
        ...
        expected_sfa = (
            1,
            -(-assignment_count // sf_tile_rows) + self.experts,
            self.k // 64,
            32,
            4,
            4,
        ...
        if self.dactivation is not None:
            ...
            expected_aux = (assignment_count, self.n)
            if (
                aux is None
                or not aux.is_cuda
        ...
        if self.output_dtype == torch.float4_e2m1fn_x2:
            output_sf_rows = -(-assignment_count // sf_tile_rows) + self.experts
            expected_output_sf = (
                1,
                output_sf_rows,
                self.output_n // 64,
