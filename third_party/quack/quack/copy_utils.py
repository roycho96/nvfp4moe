...
def tma_gather4_load(
    ...
    *,
    ...
    if multicast_mask_val is None:
        ...
        # Multicast variant: cluster-shared destination + CTA mask. The rows
        # land at the same smem offset in every masked CTA, and the transaction
        # bytes update the corresponding mbarrier in each of them (the issuing
        # CTA's peers must therefore arm the same tx count, which the ab
        # pipeline already does). ptxas (CUDA 13.0, sm_100a) accepts the
        # tile::gather4 x .multicast::cluster combination.
        ptx = (
            "cp.async.bulk.tensor.2d.shared::cluster.global.tile::gather4."
            "mbarrier::complete_tx::bytes."
            f"cta_group::{num_cta}.multicast::cluster "
...
def gather_m_get_tma_copy_fn(
    ...
    num_warps: int,
    num_cta: int = 1,
    multicast_mask=None,  # Int16 CTA mask: multicast the gathered rows cluster-wide
    is_issuer=None,  # Boolean: this CTA issues the gather4s (rank-0 of the mask group)
) -> Callable:
    ...
    tma_gather4_load_fn = partial(
        tma_gather4_load, tma_desc_ptr, num_cta=cta_group, multicast_mask=multicast_mask
    )

    def copy_fn(src_idx, dst_idx, tma_bar_ptr: cute.Pointer):
        # Multicast: only the mask group's rank-0 CTA issues; the peers arm the
        # same tx count and receive the rows + barrier updates from hardware.
        if is_issuer is None or is_issuer:
