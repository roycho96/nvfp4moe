...
def _compile_gemm_epi(
    ...
    # Mirrors quack.gemm._compile_gemm: varlen_m SFA is one M-padded batch-1
    # buffer; gather_A takes the LINEAR (T, K/64) int32 word view instead
    # (gemm_sm100.sfa_linear). SFB stays per-batch under varlen_m.
    if sf_dtype:
        if gather_A:
            mSFA = make_fake_sf_linear_tensor()
        else:
            mSFA = make_fake_sf_tensor(sf_dtype, 1 if varlen_m else (l if sf_batched else None))
        mSFB = make_fake_sf_tensor(sf_dtype, l if (varlen_m or sf_batched) else None)
    else:
        mSFA, mSFB = None, None
    post_init = None
    if post_init_attrs:
