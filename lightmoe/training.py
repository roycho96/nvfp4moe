...
from ._quantization import _DEN, TensorScale, quantize_expert_stack
from .kernels.gated import GatedBackwardEpilogue, GatedEpilogue
from .kernels.grouped.runtime import GroupedNvfp4Gemm
from .kernels.grouped.wgrad import GroupedWgrad
from .kernels.quantize.runtime import (
...
def _gkws(L, M):
    """Select tiles for the down projection and two input-gradient GEMMs."""
    if L.d <= 512 and L.I <= 512:
        dgrad2 = _GKW_D2_SMALL
    elif L.d <= 2048 and L.I <= 768:
        dgrad2 = _GKW_D2_DEFAULT
    else:
        dgrad2 = _GKW_2CTA
...
def _gate_up_config(L, M):
    """Select the gate/up tile from the average expert assignment count."""
...
class _MoEFn(torch.autograd.Function):
    ...
    def forward(
        ...
        I = L.I
        # Quantized inputs are saved for activation recomputation. The gate/up
        # projection consumes expert-ordered rows, so quantization gathers them.
        L.s_x.update(x)
        ...
        sf_rows = -(-M // 128) + L.E
        # The gate/up projection fuses gather, activation, and NVFP4 quantization.
        ...
        # The down projection folds routing weights and scales into its epilogue.
        ...
        down_scale = (L.s_h.pts * L.p_w2).reshape(1)
        down_config_default, _, _ = _gkws(L, M)
        down_config = _GKW if I <= 1024 else down_config_default
        L._grouped_gemm("fc2", d, I, down_config)(
            q_h,
            L.qb2,
            yw,
            cu,
...
class MoEExpertLayer(nn.Module):
    """NVFP4 expert layer with deterministic backward kernels."""
    ...
    def _grouped_gemm(
        ...
        if runtime is None:
            ...
            self._grouped_gemms[key] = runtime
        return runtime
    ...
    def calibrate_routed(self, x, cu, off_pad):
        """Seed activation scales from expert-major assignments."""
