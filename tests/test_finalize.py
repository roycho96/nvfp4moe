"""Check deterministic MoE gather-reduce finalization on SM100.

Checks:
  1. bitwise determinism over 20 runs;
  2. exact match against a fixed-order FP32 reference;
  3. -1 (capacity-dropped) slots contribute nothing.

    python tests/test_finalize.py
"""
...
def main():
    ...
    check(
        "exact vs fixed-order fp32 reference",
        torch.equal(out, ref),
        f"mismatched {(out != ref).sum().item()}/{out.numel()}",
    )

    base = out.clone()
    stable = all((moe_finalize(yw, slots, out, k) or torch.equal(out, base)) for _ in range(20))
