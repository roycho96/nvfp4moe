...
from real_model_stack import QwenMoEBlock, QwenNonExpert, compile_nonexpert
...
def _expert_measurement_calls(backend, x, topi, topv, dout, supports_training):
    step = [0]
...
def _stack_measurement_calls(block, x, dout):
    step = [0]

    def forward():
        step[0] += 1
        return block(x, step[0])
...
def _gradient_status(backend, x, topv):
    parameter_grads = [
        grad for parameter in backend.parameters() if (grad := parameter.grad) is not None
    ]
    return {
        ...
        "router_weight_finite": bool(topv.grad is not None and torch.isfinite(topv.grad).all()),
        "parameter_tensors_with_grad": len(parameter_grads),
        "parameter_grads_finite": bool(
            parameter_grads and all(torch.isfinite(grad).all() for grad in parameter_grads)
        ),
        ...
        "router_weight_norm": (topv.grad.float().norm().item() if topv.grad is not None else None),
        "parameter_grad_norm": (
...
def run_expert_suite(spec_key, trace_path, backends, warmup=5, iters=15):
    ...
    fixed = {
        ...
        for key in ("expert_input", "topk_index", "topk_weight", "gate_up_weight", "down_weight")
    }
    ...
    training_ref = _training_reference(spec, fixed, x_base, topi, topv_base, dout_base)
    ref = training_ref["output"]
    torch.cuda.synchronize()

    rows = []
    for name in backends:
        print(f"[benchmark] expert {spec_key} {name}: start", flush=True)
        ...
        try:
            ...
            fwd, train = _expert_measurement_calls(backend, x, topi, topv, dout, supports_training)
            ...
            row["forward"]["tokens_per_second"] = x.shape[0] * 1e3 / row["forward"]["event_ms_p50"]
            ...
            if supports_training:
                ...
                weight_grads = _crop_weight_grads(spec, backend.training_gradients())
                row["gradient_accuracy_vs_bf16_torch"] = {
                    "input": _accuracy_vs_cpu(x.grad, training_ref["input"]),
                    "router_weight": _accuracy_vs_cpu(topv.grad, training_ref["router_weight"]),
                ...
                row["train_fwd_bwd"] = {"status": "not_supported_by_this_backend_adapter"}
                row["gradient_status"] = {"status": "not_applicable"}
                row["gradient_accuracy_vs_bf16_torch"] = {"status": "not_applicable"}
            row["status"] = "ok"
        ...
        rows.append(row)
        print(
            f"[benchmark] expert {spec_key} {name}: {row['status']}"
...
def run_qwen_stack_suite(
    ...
    trace_cpu = torch.load(trace_path, map_location="cpu", weights_only=False)
    fixed = {
        key: trace_cpu[key].cuda() for key in ("gate_up_weight", "down_weight", "expert_input")
    }
    ...
    nonexpert_validation = {
        ...
        "router_topk_index_match": (validation_topi.reshape_as(trace_topi) == trace_topi)
        .float()
        .mean()
        .item(),
        "router_topk_weight": _accuracy(validation_topv.reshape_as(trace_topv), trace_topv),
    }
    ...
    for attention in attention_backends:
        for compile_mode in compile_modes:
            for backend_name in backends:
                print(
                    f"[benchmark] stack {attention}/{compile_mode}/{backend_name}: start",
                    flush=True,
                )
                ...
                try:
                    ...
                    nonexpert = QwenNonExpert(spec, trace_cpu["layer0_nonexpert"], attention)
                    ...
                    fwd, train = _stack_measurement_calls(block, x, dout)
                ...
                rows.append(row)
                print(
                    f"[benchmark] stack {attention}/{compile_mode}/{backend_name}: "
                    f"{row['status']}" + (f" ({row['error']})" if row["status"] != "ok" else ""),
                    flush=True,
                )
                block = expert = nonexpert = x = dout = fwd = train = None
                _release()
...
def run_all(
    ...
    if include_stack and "qwen3_30b_a3b" in trace_paths:
