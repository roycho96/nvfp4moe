...
def validate_blockscaled_sf(
    ...
    *,
    ...
    if sfa_linear:
        # gather_A: SFA is the LINEAR per-source-row grid (T, K/sf_vec), viewed
        # host-side as (T, K/(4*sf_vec)) int32 words so the kernel's 4-B
        # cp.async word view needs no in-kernel recast.
        assert SFA.dtype == torch.int32, (
            f"linear SFA must cross as (T, K/(4*vec)) int32 words, got {SFA.dtype}"
        )
    else:
        assert SFA.dtype == fmt_a.scale_dtype, (
            f"SFA dtype mismatch: {SFA.dtype} vs {fmt_a.name} scale {fmt_a.scale_dtype}"
        )
    assert SFB.dtype == fmt_b.scale_dtype, (
        f"SFB dtype mismatch: {SFB.dtype} vs {fmt_b.name} scale {fmt_b.scale_dtype}"
    )
    ...
    if varlen_k:
        ...
        if sfa_linear:
            # A is (T, k_storage): T source rows, gathered by A_idx. SFA carries
            # one word row per source row.
            assert k_logical % 64 == 0, (
                f"linear SFA requires K % 64 == 0 (int32 word view), got K={k_logical}"
            )
            words = k_logical // (4 * sf_vec_size)
            assert SFA.ndim == 2 and tuple(SFA.shape) == (A.shape[0], words), (
                f"linear SFA shape {tuple(SFA.shape)} != (T={A.shape[0]}, {words})"
            )
            assert SFA.stride(-1) == 1, "linear SFA rows must be contiguous"
            # Both callers already require K % (4*inst_k) == 0, which makes the
            # word row stride a multiple of 4 (= 16 B). Pinning that here, plus
            # a 16-B-aligned base, is what lets the SF producer warp read one
            # row's k-tile run as a single aligned 16-B load instead of four
            # 4-B ones (gemm_sm100._make_xpose_SFA_copy). Every producer of this
            # tensor is a fresh allocation or a row-granular slice of one, so
            # this states the existing contract rather than narrowing it.
            assert SFA.stride(0) % 4 == 0, (
                f"linear SFA word row stride {SFA.stride(0)} must be a multiple of 4"
            )
            assert SFA.data_ptr() % 16 == 0, (
                f"linear SFA base must be 16-B aligned (off by {SFA.data_ptr() % 16})"
            )
        else:
    ...
    blocked_sfs = (("SFB", SFB),) if sfa_linear else (("SFA", SFA), ("SFB", SFB))
    for name, SF in blocked_sfs:
...
def make_fake_sf_linear_tensor():
    """Fake (T, K/(4*sf_vec)) Int32 LINEAR scale-factor tensor (blockscaled
    gather_A): one word row per un-permuted source row, 4 scale bytes per
    int32 word. The row stride is dynamic (word-granular) so slices of larger
    scale buffers are accepted."""
    t, w = cute.sym_int(), cute.sym_int()
