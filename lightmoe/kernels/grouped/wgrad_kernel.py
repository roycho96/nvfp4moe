...
``grouped.scheduler`` and weight-gradient tensor mapping in ``wgrad_tma``.
"""
...
def _compute_stages(
    ...
    a_layout = sm100_utils.make_smem_layout_a(tiled_mma, mma_tiler_mnk, a_dtype, 1)
    b_layout = sm100_utils.make_smem_layout_b(tiled_mma, mma_tiler_mnk, b_dtype, 1)
    sfa_layout = blockscaled_utils.make_smem_layout_sfa(tiled_mma, mma_tiler_mnk, sf_vec_size, 1)
    sfb_layout = blockscaled_utils.make_smem_layout_sfb(tiled_mma, mma_tiler_mnk, sf_vec_size, 1)
    c_layout_stage = sm100_utils.make_smem_layout_epi(c_dtype, c_layout, epi_tile, 1)

    ab_bytes = (
        cute.size_in_bytes(a_dtype, a_layout)
        + cute.size_in_bytes(b_dtype, b_layout)
        + cute.size_in_bytes(sf_dtype, sfa_layout)
        + cute.size_in_bytes(sf_dtype, sfb_layout)
    )
    fixed_bytes = 1024 + 4 * 4 * num_tile_stage
    c_bytes = cute.size_in_bytes(c_dtype, c_layout_stage) * num_c_stage
...
class BlockScaledMoEGroupedGemmWgradKernel:
    ...
    def get_workspace_bytes(self) -> int:
        return WgradSfTensormapConstructor.get_workspace_size(
            self.input_order, self.weight_mode, self.expert_cnt
        )
    ...
    def __call__(
        ...
        global_scale_a: cute.Tensor | None = None,
        global_scale_b: cute.Tensor | None = None,
        # Discrete-only: template tensor for a single expert's output (M, N) or (M, N, 1)
        out_single_expert: cute.Tensor | None = None,
    ) -> None:

        # Public CUTLASS DSL 4.5 needs the packed-FP4 from_dlpack layout
        # workaround. Rubin and the internal DSL wheel consume the intrinsic
        ...
        if cutlass.const_expr(
            self.weight_mode == MoEWeightMode.DISCRETE and self.architecture != "sm_107"
        ):
    ...
    def helper_kernel(
        ...
        """
        from .wgrad_tma import WgradSfTensormapConstructor
    ...
    def kernel(
        ...
        # Build extension
        from .wgrad_tma import TensormapWorkspace, WgradSfTensormapConstructor
                        ...
                        if cutlass.const_expr(self.num_sf_windows_per_ab_stage == 1):
                            ...
                            for sf_window_idx in cutlass.range_constexpr(
                                ...
                                for window_k_block in cutlass.range_constexpr(
                                    ...
                                    stage_k_block = (
                                        sf_window_idx * self.num_mma_instructions_per_sf_window
                                        + window_k_block
                                    )
                                    s2t_source_coord = (
                                        None,
            ...
            while work_tile_info.is_valid_tile:
                ...
                if cutlass.const_expr(global_scale_a is not None):
                    ...
                    alpha = cute.arch.load(
                        current_scale_a_iter.llvm_ptr, cutlass.Float32
                    ) * cute.arch.load(current_scale_b_iter.llvm_ptr, cutlass.Float32)
                else:
                    alpha = None
                ...
                for subtile_idx in cutlass.range(subtile_cnt):
                    ...
                    acc_vec = cute.zeros_like(
                        tiled_copy_r2s.retile(tTR_rAcc), dtype=tTR_rAcc._dtype
                    )
                    if k_tile_cnt > 0:
                ...
                if cutlass.const_expr(not self.overlapping_accum):  # noqa: SIM102
                    if k_tile_cnt > 0:
                        acc_pipeline.consumer_release(acc_consumer_state)
                        acc_consumer_state.advance()
