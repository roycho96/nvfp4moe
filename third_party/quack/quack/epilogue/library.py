...
@functools.lru_cache(maxsize=None)
def dgated_mod(activation, *, has_scale, has_reduce, scalar_scale=False,
               scale_aux=True):
    ...
    scalar_scale=True swaps the pinned colvec scale for an inferred scalar
    operand ``gscale`` (a (1,) device tensor — the per-tensor dequant scale
    case, where every colvec entry held the same value anyway).
    scale_aux=False emits mAuxOut UNSCALED (the raw postact) while dout still
    gets the scale — callers that quantize the aux with a folded per-tensor
    scale skip the unscale round trip entirely."""
    dgate = dgate_fn_map[activation]
    sname = "gscale" if scalar_scale else "mColVecBroadcast"
    params, body = ["c"], []
    if has_scale:
        params.append(sname)
    body.append("x, y = unpack(c)")
    dout = f"acc * {sname}" if has_scale else "acc"
    body.append(f"dx, dy, out = dgate(x, y, {dout})")
    postact = f"out * {sname}" if (has_scale and scale_aux) else "out"
    if has_reduce:
    ...
    tag = (f"dgated:{activation}:s{int(has_scale)}r{int(has_reduce)}"
           f"ss{int(scalar_scale)}sa{int(scale_aux)}")
