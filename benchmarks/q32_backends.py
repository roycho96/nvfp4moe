...
def _dactivation(gate: torch.Tensor, up: torch.Tensor, dout: torch.Tensor, kind: str):
    ...
    if kind == "swiglu":
        ...
        tq = torch.tanh(q)
        act = 0.5 * g * (1.0 + tq)
        dact = 0.5 * (1.0 + tq) + 0.5 * g * (1.0 - tq * tq) * c * (
            1.0 + 3.0 * 0.044715 * g * g
        )
    else:
        raise ValueError(kind)
...
class Nvfp4MoeExpert(nn.Module):
    ...
    def forward(self, x, topi, topv, step=0):
        ...
        return self.layer(
            x, gi, cu, ps_diff, slots, off_pad=self.dispatch.off_pad
        )
...
class TEExpert(nn.Module):
    ...
    def forward(self, x, topi, topv, step=0):
        ...
        counts = mask.sum(0)
        xp, pprob, row_map, pad_offsets, target_counts = (
            te_perm.moe_permute_and_pad_with_probs(x, probs, mask, counts, self.align)
        )
...
def _dg_wgrad(dy, x, padded_counts, ks_tensor, experts):
    ...
    out = torch.zeros(
        experts, dy.shape[1], x.shape[1], dtype=torch.float32, device=dy.device
    )
    ...
    def _layout_for(self, target_counts):
        if self._layout is None:
            ...
            ks_tensor = torch.tensor(
                padded_counts, dtype=torch.int32, device=target_counts.device
            )
...
def _dg_grouped_fp4_weights(weight):
    ...
    for expert_weight in weight:
        q, sf = per_token_cast_to_fp4(
            expert_weight.contiguous(), use_ue8m0=True, gran_k=32
        )
        packed.append(q)
        scales.append(sf)
...
class TEDeepGEMMFP8FP4Expert(nn.Module):
    ...
    def _indices_for(self, target_counts):
        if self._m_indices is None:
            self._m_indices = torch.repeat_interleave(
                torch.arange(
                    self.experts, device=target_counts.device, dtype=torch.int32
                ),
                target_counts.long(),
            ).contiguous()
    ...
    def forward(self, x, topi, topv, step=0):
        ...
        counts = mask.sum(0)
        xp, pprob, row_map, pad_offsets, target_counts = (
            te_perm.moe_permute_and_pad_with_probs(
