...
def fmin(a: float | Float32, b: float | Float32, *, nan=True, loc=None, ip=None) -> Float32:
    if nan:
...
def fmax(a: float | Float32, b: float | Float32, *, nan=True, loc=None, ip=None) -> Float32:
    if nan:
...
def fmin_bf16x2(
    ...
    *,
    ...
    if nan:
        ptx_instr = "min.NaN.bf16x2 $0, $1, $2;"
    else:
        ptx_instr = "min.bf16x2 $0, $1, $2;"
...
def fmax_bf16x2(
    ...
    *,
    ...
    if nan:
        ptx_instr = "max.NaN.bf16x2 $0, $1, $2;"
    else:
        ptx_instr = "max.bf16x2 $0, $1, $2;"
...
def sigmoid_f32(a: float | Float32, fastmath: bool = False) -> float | Float32:
    """Compute the sigmoid of the input value."""
    return cute.arch.rcp_approx(1.0 + cute.math.exp(-a, fastmath=fastmath))


def silu_f32(a: float | Float32, fastmath: bool = False) -> float | Float32:
...
def silu_f32_geglu_scaled(a: float | Float32, fastmath: bool = False) -> float | Float32:
...
def compare_and_report_mismatches(
    ...
    if True:
        ...
        for i in range(num_elements):
            ...
            print(f"{i + 1:<6} {coord!s:<30} {gpu_val:<20.6f} {ref_val:<20.6f} {abs_error:<20.6f}")
...
def is_valid_mma_tiler_and_cluster_shape(
    ...
    if not (
        (not use_2cta_instrs and mma_tiler_mn[0] in [128])
        or (use_2cta_instrs and mma_tiler_mn[0] in [256])
    ):
        is_valid = False
...
def can_implement(
    ...
    if not is_valid_mma_tiler_and_cluster_shape(
        use_2cta_instrs, mma_tiler_mn, cluster_shape_mn, m_aligned, fix_pad_size
    ):
        result = False
...
def compute_stages(
    ...
    total_bytes = occupancy * (
        ab_bytes_per_stage * num_ab_stage + epi_bytes + sinfo_bytes + mbar_helpers_bytes
    )
...
def compute_stages_wgrad(
    ...
    num_ab_stage = (
        num_smem_capacity // occupancy - (mbar_helpers_bytes + c_bytes + sinfo_bytes)
    ) // ab_bytes_per_stage
    ...
    for vi in cutlass.range_constexpr(0, acc_frg.shape[0], 4):
        max_value0 = cutlass.Float32(
            cute.arch.warp_redux_sync(
                value=acc_frg[vi, 0], kind="fmax", mask_and_clamp=0xFFFFFFFF, nan=True
            )
        )
        max_value1 = cutlass.Float32(
            cute.arch.warp_redux_sync(
                value=acc_frg[vi + 1, 0], kind="fmax", mask_and_clamp=0xFFFFFFFF, nan=True
            )
        )
        max_value2 = cutlass.Float32(
            cute.arch.warp_redux_sync(
                value=acc_frg[vi + 2, 0], kind="fmax", mask_and_clamp=0xFFFFFFFF, nan=True
            )
        )
        max_value3 = cutlass.Float32(
            cute.arch.warp_redux_sync(
                value=acc_frg[vi + 3, 0], kind="fmax", mask_and_clamp=0xFFFFFFFF, nan=True
            )
        )
