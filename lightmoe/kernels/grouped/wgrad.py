"""Grouped block-scaled weight-gradient runtime."""

import torch

from .._common import jit_cache
from .wgrad_kernel import (
    BlockScaledMoEGroupedGemmWgradKernel,
)
from .wgrad_tma import MoEWeightMode, WGradInputOrder
...
def _compile(
    ...
    dev = "cuda"
    a_s = torch.zeros(m, K // 2, dtype=torch.uint8, device=dev).view(torch.float4_e2m1fn_x2)
    b_s = torch.zeros(n, K // 2, dtype=torch.uint8, device=dev).view(torch.float4_e2m1fn_x2).t()
    ...
    a_f = from_dlpack(a_s, assumed_align=16, enable_tvm_ffi=True).mark_compact_shape_dynamic(
        mode=1, stride_order=a_s.dim_order(), divisibility=16
    )
    b_f = from_dlpack(b_s, assumed_align=16, enable_tvm_ffi=True).mark_compact_shape_dynamic(
        mode=0, stride_order=b_s.dim_order(), divisibility=16
    )
    sfa_f = from_dlpack(sfa_s, assumed_align=16, enable_tvm_ffi=True).mark_compact_shape_dynamic(
        mode=1, stride_order=sfa_s.dim_order(), divisibility=4
    )
    sfb_f = from_dlpack(sfb_s, assumed_align=16, enable_tvm_ffi=True).mark_compact_shape_dynamic(
    ...
    max_active = hw.get_max_active_clusters(kernel.cluster_shape_mn[0] * kernel.cluster_shape_mn[1])
    return cute.compile(
        kernel,
        a_f,
        b_f,
...
class GroupedWgrad:
    ...
    def __call__(self, a, b_t, sfa, sfb, off_pad, out, gs_a, gs_b):
        ...
        self._fn(a_v, b_v, sfa_v, sfb_v, out, off_pad, self._ws, gs_a, gs_b, None)
