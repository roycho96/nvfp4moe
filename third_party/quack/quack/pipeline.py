...
class PipelineAsyncUmma(_PipelineIndexPhaseMixin, PipelineAsyncUmmaOg):
    ...
    def producer_cpasync_commit(self, state: PipelineState, *, loc=None, ip=None):
        """cp.async producer variant of producer_commit.

        The full barrier tracks cp.async completion instead of a plain arrive:
        every producing lane issues cp.async.mbarrier.arrive.noinc, which fires
        when that lane's outstanding async copies have landed. The barrier's
        arrive count must therefore be the producing THREAD count (32 per warp),
        exactly as for PipelineTmaCpAsyncUmma's cp.async producers.
        """
...
class PipelineTmaCpAsyncUmma(PipelineTmaUmmaOg):
    ...
    @dsl_user_op
    def producer_sts_commit(self, state: PipelineState, *, loc=None, ip=None):
        """Plain-store producer variant of ``producer_cpasync_commit``.

        A producer that fills its slice of the stage with ordinary shared
        stores (STS) has nothing for the barrier to track, so it arrives
        directly. One arrive per lane keeps the accounting identical to
        ``cp.async.mbarrier.arrive.noinc``, which also contributes one arrival
        per producing THREAD (32 per warp) — so a warp may swap between the two
        without touching the pipeline's producer arrive count.

        STS lands through the generic proxy while the UMMA consumer reads the
        stage through the async proxy (``tcgen05.cp`` / ``tcgen05.mma``), hence
        the ``fence.proxy.async.shared::cta`` before the arrive: without it the
        consumer may read pre-store bytes.
        """
        cute.arch.fence_view_async_shared(loc=loc, ip=ip)
