...
def _make_post_init(**attrs):
    """setattr hook for constexpr-like kernel knobs (stage_overrides, SF panel).

    These are trace-time attributes rather than compiled-kernel arguments, but
    every one of them is a parameter of the jit_cache'd _compile_gemm below, so
    distinct values still compile (and cache) distinct kernels.
    """
...
def _compile_gemm(
    ...
    has_ag=False,
    sfa_linear=False,  # blockscaled gather_A: SFA crosses as (T, K/64) int32 words
    debug_skip_sfa_gather=False,
    stage_overrides=None,  # SM100 (ab, epi, epi_c) stage overrides, jit-keyed
    # SM100 sfa_linear SF panel (Q28): slot count = ceil(K / tile_K), 0 = off.
    # The value is K-derived, which is what keys this compile to a K (the fake
    # operands' K is symbolic) - never pass a constant here.
    sfa_hoist_slots=0,
    sfa_hoist_pair=False,
    # SM100 sfa_linear SF fill shape (Q29): 0 = 4-B cp.async loader,
    # 1/2 = LDG.128 + register transpose + STS.128 (2 also moves the global
    # read one k-tile ahead of the acquire it feeds). Load scheduling only.
    sfa_xpose=0,
    sfa_xpose_scatter=False,
    # SM100 sfa_linear SF ring (Q30): slot count of the private SFA ring
    # (>= 2), 0 = SFA rides the AB stages. Unlike sfa_hoist_slots this is NOT
    # K-derived - the ring wraps - so a constant is correct here.
    sfa_ring=0,
):
    sm_to_cls = {
        8: GemmDefaultSm80,
    ...
    if sf_dtype is not None:
        ...
        if sfa_linear:
            mSFA = make_fake_sf_linear_tensor()
        else:
    ...
    return compile_gemm_kernel(
        ...
        a_mma_dtype=a_mma_dtype,
        b_mma_dtype=b_mma_dtype,
        debug_skip_sfa_gather=debug_skip_sfa_gather,
        post_init=_make_post_init(
            stage_overrides=stage_overrides,
            sfa_hoist_slots=sfa_hoist_slots,
            sfa_hoist_pair=sfa_hoist_pair,
            sfa_xpose=sfa_xpose,
            sfa_xpose_scatter=sfa_xpose_scatter,
            sfa_ring=sfa_ring,
        ),
    )
...
def gemm(
    ...
    debug_skip_sfa_gather: bool = False,  # diagnostic: compile without the SF gather
    # SM100 tuning knob: (ab, epi, epi_c) stage-count overrides, None entries
    # keep the heuristic (see GemmSm100._compute_stages). Pipeline depth only -
    # the K reduction order is unchanged, outputs stay bitwise.
    stage_overrides: Optional[tuple] = None,
    # SM100 blockscaled-gather knob (Q28): hoist the gathered SFA out of the AB
    # pipeline into one per-work-tile panel (slots = K / tile_K, +smem);
    # sfa_hoist_pair picks the row-major fill. Load scheduling only - the K
    # reduction order and the SF bytes are unchanged, so outputs stay bitwise.
    sfa_hoist: bool = False,
    sfa_hoist_pair: bool = False,
    # SM100 blockscaled-gather knob (Q29): 1/2 replace the SF producer warp's
    # 4-B scattered cp.asyncs with LDG.128 + register transpose + STS.128 (2
    # also issues a k-tile's read before acquiring the stage it feeds);
    # sfa_xpose_scatter de-vectorizes the stores as a control. Same bytes, same
    # stage, same K order - outputs stay bitwise.
    sfa_xpose: int = 0,
    sfa_xpose_scatter: bool = False,
    # SM100 blockscaled-gather knob (Q30): >= 2 gives the gathered SFA its own
    # ring of this many slots with its own multi-stage barrier, so the SF
    # fill's completion leaves the AB stage's ready path. Same fill, same
    # bytes, same K order - outputs stay bitwise.
    sfa_ring: int = 0,
) -> _GemmPlan:
...
def _build_gemm_plan(
    ...
    *,
    ...
    if gather_A:
        ...
        assert cluster_N == 1 or use_tma_gather, (
            "gather_A requires cluster_N=1 unless use_tma_gather (multicast gather4)"
        )
    if has_ag:
    ...
    if blockscaled:
        ...
        if gather_A:
            # Blockscaled x gather_A: the cp.async producer warps gather A rows
            # AND their LINEAR scale factors (SFA as (T, K/(4*vec)) int32 word
            # rows) directly into the blocked smem atom; SFB stays blocked TMA.
            assert varlen_m, "blockscaled gather_A requires varlen_m"
            assert device_capacity[0] in [10, 11], "blockscaled gather_A requires SM100/SM110"
            sfa_linear = True
            k_logical = fmt_a.logical_k(A.shape[-1])
            # tile_K is instruction-derived (4 * inst_k); a full last k-tile is
            # required because the SF gather issues no k-tail predicate.
            inst_k = 64 if (fmt_a.elem_bits == 4 and fmt_b.elem_bits == 4) else 32
            assert k_logical % (4 * inst_k) == 0, (
                f"blockscaled gather_A requires K % {4 * inst_k} == 0, got K={k_logical}"
            )
            if sfa_hoist:
                # One panel slot per k-tile of the WHOLE K (the mainloop's tile_K
                # is instruction-derived, 4 * inst_k). Derived from this call's K
                # so the compile key is K-specific.
                sfa_hoist_slots = k_logical // (4 * inst_k)
        ...
        sf_dtype, sf_vec_size = validate_blockscaled_sf(
            ...
            varlen_k=varlen_k,
            b_kn=b_kn,
            sfa_linear=sfa_linear,
            fmt_a=fmt_a,
            fmt_b=fmt_b,
        )
    assert not sfa_hoist or sfa_hoist_slots > 0, (
        "sfa_hoist applies to the blockscaled gather path only (linear SFA)"
    )
    assert not sfa_xpose or sfa_linear, (
        "sfa_xpose applies to the blockscaled gather path only (linear SFA)"
    )
    assert not sfa_ring or sfa_linear, (
        "sfa_ring applies to the blockscaled gather path only (linear SFA)"
    )
    ...
    compiled_fn = _compile_gemm(
        ...
        batched,
        b_kn,
        # sf_batched: linear SFA is rank-2, so derive batchedness from SFB
        # semantics instead (varlen_m keeps a per-batch rank-6 SFB).
        (True if sfa_linear else SFA.ndim == 6) if blockscaled else True,
        sfd_dtype,
        sfd_norm_const_mode,
        sfd_col_dtype,
        a_mma_dtype,
        ...
        stage_overrides=stage_overrides,
        sfa_hoist_slots=sfa_hoist_slots,
        sfa_hoist_pair=sfa_hoist_pair and sfa_hoist_slots > 0,
        sfa_xpose=sfa_xpose if sfa_linear else 0,
        sfa_xpose_scatter=sfa_xpose_scatter and sfa_linear and sfa_xpose > 0,
        sfa_ring=sfa_ring if sfa_linear else 0,
    )
