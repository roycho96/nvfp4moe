...
from .kernels.dispatch import B_MAX, CHUNK, moe_dispatch
from .kernels.epilogue import GatedEpilogue
from .kernels.finalize import moe_finalize
...
def _tile_m(routed_rows: int, experts: int, hidden: int, intermediate: int) -> tuple[int, int]:
    large = routed_rows >= 256 * experts
...
class InferenceMoE:
    ...
    def __init__(
        ...
        *,
        ...
        self.experts = _check_positive("experts", experts)
        self.topk = _check_positive("topk", topk)
        ...
        if self.topk > self.experts:
            raise ValueError("topk cannot exceed the local expert count")
    ...
    def load_packed_weights(
        ...
        e, d, i = self.experts, self.hidden_size, self.intermediate_size
        self._check_tensor(fc1, "fc1", (e, 2 * i, d // 2), torch.float4_e2m1fn_x2)
        self._check_tensor(
            fc1_sf,
            "fc1_sf",
        ...
        self._check_tensor(fc2, "fc2", (e, d, i // 2), torch.float4_e2m1fn_x2)
        self._check_tensor(
            fc2_sf,
            "fc2_sf",
            (e, -(-d // 128), i // 64, 32, 4, 4),
        ...
        self._check_weight_scale(fc1_scale, "fc1_scale")
        self._check_weight_scale(fc2_scale, "fc2_scale")
        if self._alpha1.shape != fc1_scale.shape:
            self._alpha1 = torch.empty_like(fc1_scale)
        if self._alpha2.shape != fc2_scale.shape:
            self._alpha2 = torch.empty_like(fc2_scale)
        self.qb1, self.sfb1, self.p_w1 = fc1, fc1_sf, fc1_scale
        self.qb2, self.sfb2, self.p_w2 = fc2, fc2_sf, fc2_scale
    ...
    def _validate_routed(self, x, m_indptr, padded_offsets) -> int:
        ...
        if rows <= 0 or rows > self.max_tokens * self.topk:
            raise ValueError("routed rows must be within the configured workspace")
