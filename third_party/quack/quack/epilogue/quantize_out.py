...
class BlockScaleFactorStore(EpiOp):
    ...
    def to_params(self, gemm, args):
        ...
        # Interior tiles must start on whole SF atoms (4 slots): a tile_N
        # that is a vec multiple but not an atom multiple (e.g. gated nvfp4
        # at tile_N 192 -> per-tile slot offset 6) puts the packed flush at
        # mid-atom offsets and corrupts neighboring SF bytes (measured:
        # 26% of valid SF bytes vs the tile_N 256 oracle, data unaffected -
        # docs/Q27_RESULT.md section 5). Partial LAST tiles are fine (the
        # per-subtile bounds check drops them whole); only the tile stride
        # must be atom-aligned.
        assert gemm.cta_tile_shape_mnk[1] % (4 * vec_acc) == 0, (
            f"row-direction quantized output requires tile_N to cover whole "
            f"SF atoms ({4 * vec_acc} acc columns), got tile_N "
            f"{gemm.cta_tile_shape_mnk[1]}"
        )
