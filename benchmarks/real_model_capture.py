...
import torch
from real_model_specs import ModelSpec, canonical_trace_key
...
DATASETS = {
    ...
    for item in (
        ...
        DatasetSpec("c4_en", "allenai/c4", "en"),
        DatasetSpec("finemath_4plus", "HuggingFaceTB/finemath", "finemath-4plus"),
        DatasetSpec("fineweb2_ko", "HuggingFaceFW/fineweb-2", "kor_Hang"),
        DatasetSpec(
            "stack_v3_code",
...
def _dataset_text(dataset: DatasetSpec, row: dict) -> str:
    if dataset.nested_files:
        return "\n\n".join(
            str(file.get(dataset.text_field) or "")
            for file in row.get("files") or ()
            if not file.get("is_vendor") and file.get(dataset.text_field)
        )
    return str(row.get(dataset.text_field) or "")


def _tokenize_dataset(spec: ModelSpec, seq_len: int, dataset_key: str):
    ...
    try:
        ...
        raise ValueError(
            f"unknown dataset {dataset_key!r}; choose from {sorted(DATASETS)}"
        ) from exc
    ...
    if dataset.revision is not None:
        load_kwargs["revision"] = dataset.revision
    if dataset.config is None:
        ds = load_dataset(dataset.dataset_id, **load_kwargs)
    else:
        ds = load_dataset(dataset.dataset_id, dataset.config, **load_kwargs)
    token_ids = []
    rows = []
    ...
    for row in ds:
        text = _dataset_text(dataset, row)
        if not text:
            continue
...
def _download_selected_state(spec: ModelSpec, cache_dir: str):
    ...
    index_path = hf_hub_download(spec.model_id, "model.safetensors.index.json", cache_dir=cache_dir)
    ...
    layer0_marker = ".language_model.layers.0." if spec.key.startswith("gemma4") else ".layers.0."
    selected_names = [
        name for name in weight_map if layer0_marker in name or name.endswith("embed_tokens.weight")
    ]
    ...
    shard_paths = {
        shard: hf_hub_download(spec.model_id, shard, cache_dir=cache_dir) for shard in shards
    }
...
def _expert_weights(spec: ModelSpec, state: dict[str, torch.Tensor]):
    ...
    grouped_down = [value for name, value in state.items() if name.endswith(".experts.down_proj")]
...
def capture_trace(
    ...
    input_ids, rows, token_hash, dataset = _tokenize_dataset(spec, seq_len, dataset_key)
    ...
    critical_missing = [
        name
        for name in missing
        if ("layers.0" in name or name == "embed_tokens.weight") and "experts" not in name
    ]
    if critical_missing:
...
__all__ = ["DATASETS", "DatasetSpec", "capture_trace"]
