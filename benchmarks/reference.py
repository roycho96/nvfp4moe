"""Pure-torch reference for fused gather and dual quantization.

The implementation mirrors the NVFP4 formats consumed by the LightMoE SM100
kernels and is intentionally written for clarity rather than speed.

Layout contracts:

  qdata     E2M1, two values per byte, packed along the *contraction* axis.
            Low nibble is the even index. Expert data is contiguous without
            padding between routed segments.

  SF        E4M3, one per 16 contraction elements, stored in the 128x4 blocked
            atom used by NVIDIA block-scaled GEMMs:
  ...
  varlen SF Expert b's region starts at tile index `cu_seqlens[b] // 128 + b`
            in the rm mode. Tiles are not tightly packed; the "+ b" slack keeps each expert
...
def per_tensor_scale_from_amax(amax: torch.Tensor) -> torch.Tensor:
    """Return the NVFP4 second-level scale: ``amax / (448 * 6)``."""
...
def quantize_nvfp4_fused_epilogue(x: torch.Tensor, per_tensor_scale: torch.Tensor):
    """Reference the fused SM100 epilogue's unclamped E4M3 scale contract."""
    assert x.shape[-1] % 16 == 0, f"K={x.shape[-1]} must be a multiple of 16"
    shape = x.shape
    groups = x.float().reshape(*shape[:-1], shape[-1] // 16, 16)
    block_amax = groups.abs().amax(dim=-1)
    scaled = (block_amax / F4_E2M1_MAX) / per_tensor_scale.to(torch.float32)
    sf = scaled.clamp(max=F8E4M3_MAX).to(torch.float8_e4m3fn)
    reciprocal = ((1.0 / per_tensor_scale.to(torch.float32)) / sf.float()).clamp(
        max=torch.finfo(torch.float32).max
    )
    q = (groups * reciprocal.unsqueeze(-1)).clamp(-F4_E2M1_MAX, F4_E2M1_MAX)
    code = _f32_to_e2m1_code(q.reshape(shape))
    return _pack_nibbles(code), sf
...
def quantize_nvfp4_lastdim_te(x: torch.Tensor, global_amax: torch.Tensor):
    ...
    ges = torch.where(
        amax > 0, torch.clamp(F8E4M3_MAX * F4_E2M1_MAX / amax, max=f32max), torch.ones_like(amax)
    )
    inv6 = torch.tensor(1.0, dtype=torch.float32, device=x.device) / torch.tensor(
        6.0, dtype=torch.float32, device=x.device
    )
    mult = ges * inv6
    gds = 1.0 / ges
    shape = x.shape
...
def varlen_sf_tile_offsets(cu_seqlens: torch.Tensor) -> list[int]:
    """Return each expert's 128-row-aligned scale-factor tile offset."""
...
# --------------------------------------------------------------------------
# fused gather and dual quantization
# --------------------------------------------------------------------------
...
def traffic_bytes(T: int, d: int, k: int, dual: bool = True):
    ...
    Both designs gather the same source rows, so the same L2-reuse assumption
    is applied to each. Reads cost 2*T*d bytes with full reuse and 2*M*d bytes
    without reuse.
    """
    M = T * k
